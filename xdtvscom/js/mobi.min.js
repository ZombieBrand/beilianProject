...
Mobi.autoFixGWebTop = function(f) {
	...
	if(jm("#fk-tipsLimitUv").length != 0) {
		l = l + parseInt(jm("#fk-tipsLimitUv").height())
	}
	if(f !== 2 && f !== 7 && f !== 8) {
...
Mobi.initForms = function() {
	...
	for(var c = 0; c < d.length; c++) {
		...
		if(f) {
			f = f.replace("module", "");
			if(b.getAttribute("_autoHeight") != 1 && !(jm(b).hasClass("formStyle52") || jm(b).hasClass("formStyle53") || jm(b).hasClass("formStyle35"))) {
...
Mobi.fixFontSizeDeviation = function() {
	var l = document.querySelector("#webModuleContainer"),
		r = l.querySelectorAll(".form"),
		w = [];
	for(var x = 0; x < r.length; x++) {
		var c = r[x],
			p = c.getAttribute("id"),
			j = jm(c),
			E = j.find(".jz_mergeBorderStyle").length > 0 || j.hasClass("formStyle51") || j.hasClass("formStyle52") || j.hasClass("formStyle53") || j.hasClass("formStyle54");
		if(p) {
			p = p.replace("module", "");
			var c = Mobi.getPreviewObject("module" + p);
			if(!c) {
				continue
			}
			var h = c.querySelector(".formMiddleContent"),
				f = c.querySelector(".photoModule"),
				y = jm(c).css("margin-left"),
				s = jm(c).css("margin-right"),
				q = jm(c).css("margin-top"),
				a = jm(c).css("margin-bottom"),
				t = jm(h).css("margin-left"),
				o = jm(h).css("margin-right"),
				n = jm(h).css("margin-top"),
				A = jm(h).css("margin-bottom"),
				D = jm(h).attr("_innerType"),
				d, z, v, b, m;
			if(!h) {
				continue
			}
			var C, u, B, k;
			C = c.querySelector(".formBannerTitle");
			if(C) {
				u = jm(C).find(".titleCenter");
				B = jm(C).find(".titleTextIcon");
				k = jm(C).css("height")
			}
			if(D == 0 && !E) {
				if(!Fai.top._manageMode) {
					...
					w.push(e(p, ".formMiddleContent" + p, "margin-bottom", "0.5rem"));
					w.push(e(p, ".formMiddle" + p, "padding-bottom", "0.25rem"))
				}
			}
			if(y != "0px") {
				c.style.marginLeft = parseInt(y.replace("px", "")) / _htmlFontSize + "rem"
			}
			if(s != "0px") {
				c.style.marginRight = parseInt(s.replace("px", "")) / _htmlFontSize + "rem"
			}
			if(q != "0px") {
				c.style.marginTop = parseInt(q.replace("px", "")) / _htmlFontSize + "rem"
			}
			if(a != "0px") {
				c.style.marginBottom = parseInt(a.replace("px", "")) / _htmlFontSize + "rem"
			}
			if(C && k) {
				d = k.replace("px", "") / _htmlFontSize;
				C.style.height = d + "rem"
			}
			if(C && B && u && C.getAttribute("_bannerAutoHeight") == 0) {
				if(!Fai.top._manageMode) {
					u.css("height", d + "rem");
					u.css("line-height", d + "rem");
					B.css("height", d + "rem");
					B.css("line-height", d + "rem")
				} else {
					w.push(e(p, ".formBannerTitle" + p + " .titleCenter" + p, "height", d + "rem"));
					w.push(e(p, ".formBannerTitle" + p + " .titleCenter" + p, "line-height", d + "rem"));
					w.push(e(p, ".formBannerTitle" + p + " .titleTextIcon" + p, "height", d + "rem"));
					w.push(e(p, ".formBannerTitle" + p + " .titleTextIcon" + p, "line-height", d + "rem"))
				}
			}
			if(t != "0px") {
				z = parseInt(t.replace("px", "")) / _htmlFontSize;
				if(!Fai.top._manageMode) {
					h.style.marginLeft = z + "rem"
				} else {
					w.push(e(p, ".formMiddleContent" + p, "margin-left", z + "rem"))
				}
			}
			if(o != "0px") {
				v = parseInt(o.replace("px", "")) / _htmlFontSize;
				if(!Fai.top._manageMode) {
					h.style.marginRight = v + "rem"
				} else {
					w.push(e(p, ".formMiddleContent" + p, "margin-right", v + "rem"))
				}
			}
			if(n != "0px") {
				b = parseInt(n.replace("px", "")) / _htmlFontSize;
				if(!Fai.top._manageMode) {
					h.style.marginTop = b + "rem"
				} else {
					w.push(e(p, ".formMiddleContent" + p, "margin-top", b + "rem"))
				}
			} else {
				if(n == "0px") {
					if(!jm(c).hasClass("formStyle41") || !jm(c).hasClass("formStyle50")) {
						if(!Fai.top._manageMode) {
							jm(f).css("marginTop", "0")
						} else {
							w.push(e(p, ".photoModule", "margin-top", "0"))
						}
					}
				}
			}
			if(A != "0px") {
				m = parseInt(A.replace("px", "")) / _htmlFontSize;
				if(!Fai.top._manageMode) {
					h.style.marginBottom = m + "rem"
				} else {
					w.push(e(p, ".formMiddleContent" + p, "margin-bottom", m + "rem"))
				}
			} else {
				if(A == "0px") {
					if(!Fai.top._manageMode) {
						jm(f).css("marginBottom", "0");
						jm(".formMiddle" + p).css("paddingBottom", "0")
					} else {
						w.push(e(p, ".photoModule", "margin-bottom", "0"));
						w.push(e(p, ".formMiddle" + p, "padding-bottom", "0"))
					}
				}
			}
		}
	...
	if(w.length > 0) {
		Mobi.setCtrlStyle("mobiStyleModule", w.join("\n"))
	}

	function e(I, F, G, H) {
		return "#module" + I + " " + F + "{" + G + ":" + H + ";}"
	}
};
Mobi.topContentFixedAutoHidden = function(b) {
	if(b == 0 || b == 2) {
		...
		d.addEventListener("touchstart", function(j) {
			h.y1 = j.touches[0].pageY
		}, false);
		d.addEventListener("touchmove", function(j) {
...
Mobi.onlineServiceInputWord = function() {
	...
	var e = jm("input[type='text'], input[type='email'], input[type='tel'], input[type='number'], textArea"),
		a = document.getElementById("webCustomerServiceBox"),
		d = jm("#wxPublicAttentionService"),
		f = "",
		b = "",
		c = false;
	if(a) {
		c = a.querySelectorAll(".service").length > 0 ? true : false
	}
	if(!a || !c && d.length < 1) {
		return
	}
	e.each(function(h, j) {
		jm(j).off("blur.inputted").off("focus.inputted").on("blur.inputted", function() {
			a.style.display = b;
			d[0].style.display = f
		}).on("focus.inputted", function() {
			b = jm("#webCustomerServiceBox").css("display");
			f = d.css("display");
...
Mobi.createFullScreenDiv = function(b, c, a) {
	...
	jm("#fullScreenDivCotainer").append("<div id='" + b + "' class='fullScreenDiv webBackground webContainerBox moduleContent categoryModel refreshBase'></div>");
...
Mobi.customPopup = function(n) {
	...
	c.append(e.join(""));
	if(n.callback) {
		c.find(".J-prompt-callback").on("click", function() {
	...
	if(n.cancelCallback) {
		c.find(".J-prompt-cancel").on("click", function() {
			m.removeClass("mobiPreludeShow");
			c.removeClass("mobiPromptShow");
			l.removeClass("mobiPromptBgShow");
			setTimeout(function() {
...
Mobi.showPanel = function(f, a) {
	...
	if(j.length == 0 || v == undefined || redo) {
		...
		if(b) {
			Mobi.bindTouchAndClickEvent(jm("#webPagePanel" + o + " .webPagePanelBg"), function(x) {
		...
		for(var r = 0; r < h.length; r++) {
			var k = h[r].trim();
			Mobi.bindTouchAndClickEvent(jm(k), function(x) {
				v.removeClass("webPagePanelShow");
				jm("#g_body").css({
					overflow: "",
					height: "",
		...
		Mobi.bindTransitionEnd(v, function() {
	...
	e.on("click", function() {
		...
		if(e.hasClass("faisco-icons-downward1")) {
			...
			if(h.length > 0) {
				...
				c.find(".J_navMask").on("click", function() {
					e.trigger("click")
				})
			}
		} else {
...
Mobi.initProductGroup = function(l, t, b) {
	...
	w.off(".productNewGroup").on("click.productNewGroup", function() {
		...
		var C = jm(this),
			...
			D = jm("#sed_" + E),
			A, F;
		if((D.html() === "" || D.find(".J_sedGroundFlag").html() === "") && r) {
...
(function(f, a) {
	...
	function h() {
		...
		var k;
		f(j.targets).find(".quick_in").unbind("click.quickIn").bind("click.quickIn", function(n) {
			k = f(this);
		...
		f(j.targets).off(".quickIn").on("mouseenter.quickIn", function() {
...
(function(f, c, d) {
	...
	function k(o) {
		...
		f(o).off(".simpleDrag");
		f(o).on("mousedown.simpleDrag", function(p) {
			b(p, n, this)
		}).on("touchstart.simpleDrag", function(p) {
			b(p, n, this)
		}).on("mouseup.simpleDrag", function(p) {
			e(p, n, this)
		}).on("touchend.simpleDrag", function(p) {
			e(p, n, this)
		}).on("mousemove.simpleDrag", function(p) {
			j(p, n, this);
			p.preventDefault()
		}).on("touchmove.simpleDrag", function(p) {
			j(p, n, this);
			p.preventDefault()
		})
	}
		...
		if(c.type === "confirm") {
			...
			var m = b.extend({
				...
				beforeShow: function(l) {
					j.touch(b(".fkarea-ui-confirm-submit", l), function() {
						c.callback && c.callback.call(j, "yes", l);
						b("body").css({
							overflow: "auto"
						})
					});
					j.touch(b(".fkarea-ui-confirm-no", l), function() {
	...
	b.prototype = {
		...
		bindEvent: function() {
			...
			this.trigger[d.settings.eventName](function(m) {
				...
				c.confirm('<div class="fkarea-ui-scroller-mask"><div id="' + d.id + '" class="ui-scroller">' + l + "<p></p></div></div>", d.buttons, function(e, p) {
					if(e == "yes") {
						d.submit()
					}
					if(e == "no") {
				...
				d.scroller.children().bind("touchstart", function(p) {
					o = (p.changedTouches || p.originalEvent.changedTouches)[0].pageY
				});
				d.scroller.children().bind("touchmove", function(s) {
		...
		f: function(l) {
			...
			if(o.index !== 0 && o.value[o.index - 1] == "0" && this.init == 0) {
				...
				for(var k = o.init, n = s.length; k < n; k++) {
					var m = s[k].pid || 0;
					var d = s[k].id || 0;
					var r = "";
...
Mobi.ajaxClick = function(h, f, c) {
	...
	loadingAjaxDom = function(e) {
		...
		jm.ajax({
			...
			success: function(m, q, o) {
				var l = o.getResponseHeader("wafUpdate");
				if(l && l == "true") {
					window.location.reload()
				}
				d = true;
				closeLoading(e);
				var m = jm.parseJSON(m);
				if(m.success) {
					var n = jm(m.domStr).find(".middleCenter" + e).html();
					var p = jm("#module" + e).find(".middleCenter" + e);
					p.html(n);
					jm.fkEval.call(window, m.scripts);
...
Mobi.fixWXPublicAttentionTip = function(b, e, a) {
	...
	f.bind("click", function() {
		...
		jm("#wxPagePic").bind("mouseup", function(k) {
			k.stopPropagation();
			k.preventDefault()
		})
	});
...
Mobi.fixNewsJumpLink = function(c, a) {
	var b = jm("#module" + c);
	b.find(".newsLine").on("click.link", function(d) {
...
Mobi.fixNewsDetailVideo = function() {
	var a = Fai.top.navigator.userAgent;
	if(!(/(iPhone)/i.test(a))) {
		return
	}
	jm("video").each(function(b, c) {
		jm(c).on("progress", function() {
			c.played.length < 1 && c.play()
		})
	})
};
Mobi.jumptoHttpInCodeModule = function() {
	...
	if(Fai.top.curHttpInCode == Fai.top.httpInCodeModuleList.length - 1) {
		...
		++Fai.top.curHttpInCode
	}
	var b = Fai.top.httpInCodeModuleList[Fai.top.curHttpInCode];
	...
	if(b == 0) {
		...
		if(b == -1) {
			...
			var a = document.getElementById("module" + b).offsetTop;
			if(document.getElementById("module" + b).offsetParent.offsetTop > 0) {
				a += document.getElementById("module" + b).offsetParent.offsetTop
			}
			$("html, body, #g_main, #web").animate({
				scrollTop: a + $("#webHeaderBox").height() + $("#webBannerBox").height() + document.getElementById("g_web").offsetTop
			}, {
				duration: 500,
				easing: "swing"
			})
...
Mobi.showNavItemContainer = function() {
	...
	if(m > t) {
		...
		h.on("click.nav", ".navItem", function() {
			var j = jm(this),
				H = j.offset(),
...
Mobi.navSwipeMenu = function(a) {
	...
	Mobi.bindTouchAndClickEvent(jm("#" + c), function() {
		e();
		return false
	}, "show");
...
Mobi.initNavSubMenu = function(l, a, b) {
	var h = [],
		f = [],
		c, j = (Fai.top._manageMode && Fai.top._columnStyleJson.s) || a,
		n = (Fai.top._manageMode && Fai.top._other.templateInfo.templatePresetIndex) || b;
	for(var k in l) {
		nav = l[k];
		c = nav.children || nav.cil, f.push(nav.id);
		if(Fai.top._manageMode) {
	...
	for(var e = 0; e < f.length; e++) {
		(function(p) {
			...
			if(q.length < 1) {
				...
				r.off("click.subMenu").on("click.subMenu", function() {
					j = (Fai.top._manageMode && Fai.top._columnStyleJson.s) || a, n = (Fai.top._manageMode && Fai.top._other.templateInfo.templatePresetIndex) || b;
	...
	jm("body").off("click.subMenu").on("click.subMenu", function(o) {
...
Mobi.initOfficalNavSubMenu = function(j, k, s) {
	...
	for(var w in j) {
		nav = j[w];
		f = nav.children || nav.cil, p.push(nav.id);
		if(Fai.top._manageMode) {
			if(nav.children && nav.children.length > 0) {
	...
	for(var t = 0; t < p.length; t++) {
		(function(y) {
			...
			x.on(b + ".subMenu", function(K) {
				...
				if(P == "panel" || P == "panel2") {
					jm(".parent_menu").on("click.nav", function() {
						Mobi.hideOfficalNavSubMenu()
					})
				}
				var H = jm("#navItem_" + D);
	...
	jm("body").off(b + ".subMenu2").on(b + ".subMenu2", function(x) {
	...
	if(n == "bubble_horizon") {
		e.off("scroll.subMenu").on("scroll.subMenu", function() {
			if(e.find(".subMenuOpen").length > 0) {
				Mobi.hideOfficalNavSubMenu()
			}
		})
	}
	l.on(b + ".subMenu", function() {
		Mobi.hideOfficalNavSubMenu()
	});
	jm("#navExistOffPanel").off(b + ".subMenu").on(b + ".subMenu", function() {
		Mobi.hideOfficalNavSubMenu()
	});
	if(h == 8 || h == 6 || h == 1 || h == 0) {
	...
	function v(x, B) {
		var z = "ontouchstart" in window,
			A = z ? "touchend" : "click",
			y = false;
		x.find("#navbar_" + B).off(A + ".navHref").on(A + ".navHref", function() {
			var C = jm(this).attr("href");
			if(C && jm.trim(C) == "javascript:;") {
				return
			}
			if(y) {
				y = false;
			...
			jm.setCookie("navId", B);
			if(Fai.top._manageMode) {
				if(Fai.top.Mobi.checkSaveBar()) {
					return false
				}
			}
			C = Mobi.fixJzmHomeIframeParam(jm(this), C);
...
Mobi.siteFormAdd = function(a, s, q, h, e) {
	...
	if(r.length > 0) {
		jm.each(r, function(L, J) {
			...
			N.type = x;
			if(x == 0) {
				var P = jm("#M" + a + "F" + s + "siteFormInput" + G).val();
				if(H && P == "") {
					Mobi.ing(jm.format(LS.siteFormSubmitInputIsEmpty, jm.encodeHtml(S)));
					c = false;
					return false
				}
				if(P.length > n) {
					Mobi.ing(jm.format(LS.siteFormSubmitInputMaxLength, jm.encodeHtml(S), n));
					c = false;
					return false
				}
				N.val = P
				...
				if(x == 1) {
					var t = jm("#M" + a + "F" + s + "siteFormTextArea" + G).val();
					if(H && t == "") {
						Mobi.ing(jm.format(LS.siteFormSubmitInputIsEmpty, jm.encodeHtml(S)));
						c = false;
						return false
					...
					if(t.length > f) {
						Mobi.ing(jm.format(LS.siteFormSubmitInputMaxLength, jm.encodeHtml(S), f));
						c = false;
						return false
					}
					N.val = t
					...
					if(x == 2) {
						var z = document.querySelectorAll("input[name='M" + a + "F" + s + "siteFormRadioR" + G + "']:checked");
						if(H && z.length == 0) {
							Mobi.ing(jm.format(LS.siteFormSubmitCheckIsEmpty, jm.encodeHtml(S)));
							c = false;
							return false
						}
						if(z.length > 0) {
						...
						if(x == 3) {
							var B = document.querySelectorAll("input[name='M" + a + "F" + s + "siteFormCheckboxR" + G + "']:checked");
							if(H && B.length == 0) {
								Mobi.ing(jm.format(LS.siteFormSubmitCheckIsEmpty, jm.encodeHtml(S)));
								c = false;
								return false
							...
							if(x == 4) {
								var F = document.getElementById("M" + a + "F" + s + "siteFormSelect" + G),
									I = F.options[F.selectedIndex].value;
								if(H && (I.length == 0 || I == "none")) {
									Mobi.ing(jm.format(LS.siteFormSubmitCheckIsEmpty, jm.encodeHtml(S)));
									c = false;
									return false
								}
								...
								if(x == 5) {
									...
									if(x == 6) {
										var P = jm("#Mobi" + a + "Calendar" + G + "Pluginsinput").val(),
											y = J.dateSetting ? J.dateSetting.type : 0,
											D = y == 1 ? jm("#Mobi" + a + "Calendar" + G + "Plugins_endinput").val() : "";
										if(H && (P == "" || (y == 1 && D == ""))) {
											Mobi.ing(jm.format(LS.siteFormSubmitCheckIsEmpty, jm.encodeHtml(S)));
											c = false;
											return false
										...
										if(x == 7) {
											var u = jm("#module" + a).find("#siteForm" + a + "fileName" + G);
											var O = {};
											if(H && u.attr("_tmpFileId") == "") {
												Mobi.ing("请选择文件上传");
												c = false;
												return false
											}
											if(u.attr("_tmpFileId") == "") {
												...
												N.val = u.attr("_fileId");
												O.tmpFileName = u.attr("_tmpFileName");
												O.fileId = N.val;
												O.tmpFileId = u.attr("_tmpFileId");
												m.push(O)
											}
										} else {
											if(x == 8) {
												var R = jm("#M" + a + "F" + s + "siteFormPhoneInput" + G).val();
												var C = J.phoneNumberLength;
												if(!C) {
													C = "7-11"
												}
												var Q = C;
												...
												if(H) {
													if(R == "") {
														...
														if(!jm.isLimitedRangeNationMobile(R, Q, A)) {
															Mobi.ing(jm.format(LS.newMobileNumRegular, C));
															c = false;
															return false
														}
													}
												...
												if(x == 9) {
													var M = jm("#M" + a + "F" + s + "siteFormMailInput" + G).val();
													if(H) {
														if(M == "") {
															Mobi.ing(jm.format(LS.siteFormSubmitInputIsEmpty, jm.encodeHtml(S)));
															c = false;
															...
															if(!jm.isEmail(M)) {
																Mobi.ing(jm.format(LS.memberSignupUserAddItemCorrect, jm.encodeHtml(S)));
																c = false;
																return false
															}
														}
													...
													if(x == 10) {
														var w = jm("#M" + a + "F" + s + "siteFormIndentityInput" + G).val();
														if(H) {
															if(w == "") {
																Mobi.ing(jm.format(LS.siteFormSubmitInputIsEmpty, jm.encodeHtml(S)));
																c = false;
														...
														if(x == 11) {
															var v = jm("#M" + a + "F" + s + "siteFormAddressInput" + G).val();
															if(H && v == "") {
																Mobi.ing(jm.format(LS.siteFormSubmitInputIsEmpty, jm.encodeHtml(S)));
																c = false;
																return false
...
Mobi.siteFormPhoneValidate = function(d, a) {
	...
	e.keyup(function(f) {
		var h = jm(this);
		h.off("mousewheel.disabledScroll").on("mousewheel.disabledScroll", function(j) {
			j.preventDefault()
		});
		h.get(0).addEventListener("DOMMouseScroll", function(j) {
			j.preventDefault()
		}, false);
...
Mobi.codeInsert = function(f, a) {
	var d = /document.write/g;
	f = f.replace(d, "Mobi.voidFun");
...
Mobi.initQQLogin = function(h, e, c, b, d, f, a) {
	jm("#" + f).off("click").on("click", function() {
				...
				if(result.active) {
					...
					if(result.rt == -3) {
						if((typeof(faiOpenId) != "undefined") && faiOpenId) {
							Mobi.memberOtherLoginFill(optionList, nickName, openId, headImg, 5, result.mobileCtNameList, result.signNeenMobile, url, faiOpenId)
						} else {
...
Mobi.memberOtherLoginFill = function(j, a, c, l, f, e, h, b, n) {
	var m = [];
	...
	m.push("<input type='button'  class='g_button sendIcon signupSubmit submitIcon' onclick='Mobi.memberOtherLoginSubmit(\"" + a + '", ' + f + ', "' + c + '", "' + b + '", "' + l + '", "' + n + "\")' value='" + LS.login + "'/>");
	m.push("</div>");
...
Mobi.bindOtherAcctFirstSubmit = function(d, a, b, e) {
	...
	info = {}, data = "";
	acct = jm("#bindUser").val(), pwd = jm("#bindPassword").val();
	if(acct == null || acct == "") {
...
Mobi.memberOtherLoginSubmit = function(name, loginType, openId, url, avator, faiOpenId) {
	...
	jm.ajax({
		...
		data: "info=" + jm.encodeUrl(jm.toJSON(info)) + "&loginType=" + loginType + "&faiOpenId=" + encodeURIComponent(faiOpenId) + "&openId=" + encodeURIComponent(openId) + "&unionId=" + encodeURIComponent(unionId),
		error: function() {
			Mobi.ing(LS.memberSignupRegisterError)
		},
		success: function(result) {
...
Mobi.memberSignupSubmit = function(b, l, u) {
	...
	jm.ajax({
		...
		success: function(w) {
			...
			if(x.success) {
				if(x.active) {
					Mobi.memberActive(t.email, x.acct, b, function() {})
				} else {
					if(x.rt == 0) {
						Fai.top.location.href = "login.jsp?errno=-303"
					}
			...
			callback: function() {
				jm.ajax({
					...
					success: function(s) {
						...
						if(t.success) {
							if(t.active) {
								Mobi.memberActive(d.email, t.acct, b, function() {})
							} else {
								if(t.rt == 0) {
									Fai.top.location.href = "login.jsp?errno=-303"
								}
...
Mobi.getSignMobileCode = function(b) {
	if(Fai.top._manageMode) {
		Mobi.ing("您目前处于网站管理状态，请先点击网站右上方的“退出”后再获取短信验证码。");
		return
	}
	var a = jm("#mobile").val(),
		c = jm("#mobileCt").val(),
		f = jm("#memberSignupCaptcha").val(),
		d = "";
	...
	if(Fai.top._encryptType) {
		if(typeof(a) != "undefined" && Fai.top._faiEncryptKey && Fai.top._encryptType > 0) {
			var e = new JSEncrypt();
			e.setPublicKey(Fai.top._faiEncryptKey);
			if(f) {
				f = e.encrypt(f)
			}
			if(Fai.top._encryptType == 1) {
				e = new JSEncrypt1()
			} else {
				if(Fai.top._encryptType == 2) {
					e = new JSEncrypt2()
				} else {
					if(Fai.top._encryptType == 3) {
						e = new JSEncrypt3()
					}
				}
			}
	...
	jm.ajax({
		...
		data: "mobile=" + a + "&mobileCt=" + c + "&validateCode=" + f + "&mCode=" + d,
		error: function() {
...
Mobi.memberAddrInfoOpera = function(b, c, w, t) {
	...
	jm("#addrInfoList").delegate(".edit, .info_name", "click", function() {
		s = false;
		jm("#addrInfoList").hide();
		jm("#editAddrInfo").show();
	...
	jm("#addrInfoList").delegate(".selectedDiv label", "click", function() {
		A = "setDefault";
		var J = 0;
		if(k.length == 1) {
			return
	...
	var z = false;
	jm("#editAddrInfo").delegate(".orderSaveAddrInfo", "click", function() {
		z = true;
		var T = true;
		var N = 0;
		if(k.length == 0) {
	...
	jm("#addrInfo").delegate(".cutOut", "click", function() {
		var J = jm(this);
	...
	jm("#areaBox").unbind("click").bind("click", function(I) {
	...
	jm("#areaBoxInput").unbind("click").bind("click", function() {
		if(jm("#allArea").val() == "cn") {
			o();
			jm("#areaBox1").toggle();
			jm("#areaBox2").hide();
	...
	jm("#province_box .group_item").bind("click", function() {
		jm("#areaBox1 .city_content").remove();
		jm("#areaBox1 .county_content").remove();
	...
	jm("#city_box").delegate(".group_item", "click", function() {
		jm("#areaBox1 .county_content").remove();
	...
	jm("#county_box").delegate(".group_item", "click", function() {
		jm("#areaBox1 .street_content").remove();
	...
	jm("#street_box").delegate(".group_item", "click", function() {
		var H = jm(this).attr("cname");
		var J = jm(this).attr("cid");
	...
	jm("#sec_box").delegate(".group_item", "click", function() {
		jm("#areaBox2 .thd_content").html("");
		var H = jm(this).attr("cname");
		var J = jm(this).attr("cid");
	...
	jm("#thd_box").delegate(".group_item", "click", function() {
		var H = jm(this).attr("cname");
		var J = jm(this).attr("cid");
		var K = jm("#areaBoxInput").attr("lastArea");
		var I = jm("#areaBoxInput").attr("lastAreaType");
...
Mobi.modifyListener = function(c, b) {
	...
	jm(".modifyArea li").click(function() {
		...
		if(Fai.top._templateFrameId >= 2000 && Fai.top._templateFrameId < 3000) {
			...
			jm(".form .loginHeader .icon-gClose").off("click.gc").on("click.gc", function() {
...
(function(r, m, c) {
	...
	function e() {
		Mobi.onTouchAndClickEvent(".cancelCollection", function(z) {
			var x = $(this);
			var y = {
				textClass: "confirm",
		...
		r(".pdName,.pdPrice,.pdImg").click(function() {
...
Mobi.initWithDrawBorder = function() {
	var a = jm(".withdrawPanel .withdrawBtn").css("color");
	jm(".withdrawPanel .withdrawBtn").css("border", "1px solid " + a)
};
...
Mobi.loadPromoOrders = function(b) {
	...
	if(b) {
		jm(".switches .myOrders").off().on("click", function() {
		...
		jm(".switches .othersOrders").off().on("click", function() {
			jm(this).addClass("highlight");
			jm(this).siblings().removeClass("highlight");
	...
	if(b != null && b.length > 0) {
		...
		jm.each(b, function(d, e) {
			...
			if(e.itemList.length > 3) {
				jm.each(e.itemList, function(h, f) {
					if(h > 2) {
...
Mobi.promoWithdraw = function() {
	...
	var f = jm(".promoteWithdrawLocater"),
		d = null;
	html = "";
	...
	function a(n, q) {
		if(n == "editCusInfo" || n == "editAddrInfo") {
			if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
				jm("#" + n).find("input,select,textarea").bind("blur", function() {
					document.querySelector("body").scrollTop = 0
				});
				jm("#" + n).find("input,select,textarea").bind("focus", function() {
				...
				jm("#areaBox").bind("touchmove", function(s) {
					s.preventDefault()
				});
				jm("#areaBox .J_areaHead").bind("touchmove", function(s) {
					s.preventDefault()
				});
				jm("#areaBox1").bind("touchmove", function(s) {
					s.stopPropagation()
				});
				jm("#areaBox2").bind("touchmove", function(s) {
					s.stopPropagation()
				})
			}
		}
...
Mobi.bindWithdrawFormCtl = function(a) {
	...
	if(c != null) {
		if(a === 0) {
			...
			f.push(k);
			f.push(d);
			jm.each(f, function(l, m) {
				jm(this).off().on("input", function() {
					var q = b.val(),
						p = k.val(),
						o = d.val();
			...
			jm("#promoWithdrawBtn").off().on("click", function() {
				if(!e) {
					Mobi.ing(h);
					return
				} else {
...
Mobi.showWithdrawSuccPage = function(a) {
	var b = jm(".promoteWithdrawLocater"),
		d = "",
		c = "提现申请已提交，等待商家审核";
	if(!a) {
		c = "提现申请提交失败，请稍后重试"
	...
	d += "</div>";
	b.append(d);
	jm("#webModuleContainer").find(".loginHeader").find(".g_close").off().on("click", function() {
		location.href = "promoterIndex.jsp"
	});
	jm(".withdrawSubmitSucc .withdrawSubComplete").off().on("click", function() {
		location.href = "promoterIndex.jsp"
	})
};
		...
		n.onload = function() {
			...
			s.onload = function() {
				...
				if(f) {
					...
					jm(".invitePosterWrapper .posterBody").append("<div class='posterBodyBtn'>长按保存图片</div>")
				} else {
					...
					jm(".promoPosterWrapper .posterBody").append("<div class='posterBodyBtn'>长按保存图片</div>")
				}
			}
		}
	}
...
Mobi.loadCouponList = (function(j) {
	...
	function r() {
		...
		Mobi.onTouchAndClickEvent(".toUseCoupon", function() {
...
(function(o, k, f) {
	...
	j.initBind = function() {
		var E = this;
		t = o(".picArea .coverImg"), q = o(".picArea"), n = o("#headPic");
		n.on({
			load: function() {
...
Mobi.mobiMemberUploadImgHtml = function(h, d, e, f) {
	...
	jm("#" + h).parent(".modifyBtn").on("click", function() {
...
Mobi.initPhotoImageSwipe = function(b, f) {
	var c = Mobi.getJQMobi();
	var e = Mobi.getPreviewObject(b),
		d = e.querySelector(".imgName");
	if(!e) {
		return
	}
	if(f == 1) {
		Mobi.initScaleImage(e)
	}
	if(f == 2) {
		Mobi.initDefScaleImage(e)
	}
	if(Fai.top._manageMode) {
		var a = Fai.top.$("#mobiReviewPage").width();
		Fai.top.$(e).css("width", a);
		if(f == 1 || f == 0) {
			Fai.top.$(e).find(".stretchImg").css("width", a)
		}
	}
	c("#" + b).swipehandle(e, {
		isOpenNextAndPrevious: true,
		callback: function(m, h) {
			var l = h.querySelector("img"),
				k = l.getAttribute("lazysrc");
			if(!!k && !l.src) {
				l.src = k;
				l.onload = function() {}
			}
...
Mobi.initModulePhotoDetailSwipe = function(x, v, l, t) {
	...
	var e = {
		...
		w = f.attach(k, e);
	w && w.addEventHandler(f.EventTypes.onCaptionAndToolbarShow, function(j) {
		if(this.scroll) {
			this.scroll.destroy()
		}
	...
	if(l || t) {
		w.addEventHandler(f.EventTypes.onDisplayImage, function(A) {
	...
	if(navigator.userAgent.indexOf("baidubrowser")) {
		jm("body").off("touchstart.photoSwipe").on("touchstart.photoSwipe", function(j) {})
	}
	...
	jm.ajax({
		...
		success: function(p) {
			...
			var o = {
				...
				n = c.attach(l, o);
			n && n.addEventHandler(c.EventTypes.onCaptionAndToolbarShow, function(q) {
				if(this.scroll) {
					this.scroll.destroy()
				}
				this.scroll = new iScroll("wrapperBasic")
			...
			n.addEventHandler(c.EventTypes.onDisplayImage, function(u) {
...
Mobi.photoCrossedSlideSecSwipe = function(v, b, h) {
	var s = Mobi.getPreviewObject(b),
		a = Mobi.getPreviewObject(h),
		f = s,
		r = parseInt(jm(a).width()),
		c = 0,
		w = jm(".photoCrossedSlideSec", a);

	function j(y) {
		var x = 0;
		x += parseFloat(y.width());
		x += (parseFloat(y.css("padding-left")) + parseFloat(y.css("padding-right")));
		x += (parseFloat(y.css("border-left-width")) + parseFloat(y.css("border-right-width")));
		x += (parseFloat(y.css("margin-left")) + parseFloat(y.css("margin-right")));
		return x
	}
	if(w.length > 0) {
		c = j(jm(w[0])) * w.length
	...
	if(c > r) {
		...
		if(jm.os.supportsTouch) {
			...
			var n, m, l, p, o, u = jm("#" + h),
				q = jm("#" + b);
			u.off("touchstart.photoCross").on("touchstart.photoCross", function(x) {
				var y = x.touches.length > 0 ? x.touches[0] : x.chengedTouches[0];
				m = y.pageX;
				l = y.pageY;
				u.off("touchmove.photoCross").on("touchmove.photoCross", e);
				u.off("touchend.photoCross").on("touchend.photoCross", t)
			});

			function e(z) {
				var A = z.touches.length > 0 ? z.touches[0] : z.chengedTouches[0];
				p = A.pageX;
				o = A.pageY;
				var y = Math.abs(p - m),
					x = Math.abs(o - l);
			...
			function t(x) {
				n = m = l = p = o = undefined;
				u.off("touchmove.photoCross", e);
				u.off("touchend.photoCross", t)
			}
			...
			q.children().off("touchmove.banIScroll").on("touchmove.banIScroll", function(x) {
				if(n === true) {
					x.stopPropagation()
				}
			})
...
Mobi.initPhotoGroupSwipe = function(b) {
	...
	e.find(".J_preBtn").on("click", function() {
		j.prev()
	});
	e.find(".J_nextBtn").on("click", function() {
		j.next()
	})
};
...
Mobi.photoCardInit = function(t) {
	...
	j.each(function(x, y) {
		...
		s.on("webkitAnimationStart mozAnimationStart MSAnimationStart oanimationstart animationstart", function(z) {
			b.data("animated", "running")
		}).on("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function(z) {
...
(function(e, j, f) {
	e.photoGroup = j.extend({}, e.photoGroup, {
		bindInteractive: c,
		unBindInteractive: h
	});
	j(q);
	...
	function n(t, s) {
		...
		r.on("mousedown.photoGroup", function() {
			$("#webModuleContainer").sortable("disable")
		}).on("mouseup.photoGroup", function(u) {
...
(function(a, d, b) {
	...
	a.photoGroup = d.extend({}, a.photoGroup, {
		utils: l
	});
...
(function(d, j, m, h) {
	...
	j(window).load(q);
	d.photoGroup = j.extend({}, d.photoGroup, {
		bindWithEditLayer: b
	});
	...
	function b(w, t) {
		...
		w.on(x.moduleMouseEnter, function(y) {
			c(w)
		}).on(x.moduleMouseLeave, function(y) {
			c(w, false)
		});
		if([4, 5].indexOf(u) > -1) {
			r(w, t)
	...
	function c(w, s) {
		...
		u.on(x.imgMouseEnter, function(D) {
			var A = j(this),
		...
		}).on(x.imgMouseLeave, function(z) {
			var y = j(this);
			$img = d.photoGroup.utils.getImgByCard(y), flag = y.data("fk-is-enter");
			if(flag !== "true") {
				return
			}
			p.remove($img, w);
	...
	function r(u, s) {
		...
		u.on(v.transBefore, function(w) {
			k(u)
		}).on(v.transAfter, function(w) {
			f(u)
		})
	}
	...
	function o(u, v, s) {
		...
		t.on(w.wrapMousemove, function(x) {
			v.moveX = x.clientX;
			v.moveY = x.clientY
		})
	}
...
(function(b, e, c) {
	...
	b.photoGroup = e.extend({}, b.photoGroup, {
		zoomSlider: h
	});
	...
	function d(q, u, A, r) {
		...
		v.addClass(l.active);
		u.isTrans = true;
		v.off(t.transEnd).on(t.transEnd, function(C) {
...
(function(a, b, f) {
	...
	function e() {
		...
		b(".formStyle48").each(function(h, j) {
			b(j).on("mouseenter.bindEditLayer", function() {
				d(b(this))
	...
	function d(j, h) {
		...
		k.on("mouseenter.editLayer", function() {
			var s = b(this),
...
(function(e, a, b) {
	var f = a.prototype,
		c, l, h, d;
	f.init = function() {
		c = this.sessionMid, l = this.collectionList, h = this.pid, d = this._manageMode;
		m();
		j();
	...
	function m() {
		Mobi.bindTouchAndClickEvent(e(".shareBlock"), function(n) {
			if(c == 0) {
				if(d) {
...
Mobi.initProductOptions = function(b) {
	b = b || {};
	var k = b.optionsAmountList;
	var A = b.isRelateAmount;
	var d = b.realMallPrice;
	var x = b.limitAmount_options;
	var p = b.productRestrictInfo;
	var w = b.choiceCurrencyVal;
	var a = b.entry;
	var y = b.productId;
	var D = b.moduleId;
	var j = b.weightTypeStr;
	var v = b.isGroupBuy;
	var r = b.groupPrice;
	var n = b.isCutPrice;
	var q = b.cpMinPrice;
	var e = b.isSecKill;
	var m = b.secKillPrice;
	var C = false;
	jm.each(k, function(G, H) {
		if(H.t == null) {
			C = true;
			return false
		}
	...
	var f = jm(".J-op-box"),
		...
		o = "";
	if(a == 1) {
		f = jm("#productMallOptionPanel" + y + " .J-op-box");
		B = jm("#productMallOptionPanel" + y + " .mallOptionImg"), z = f.find(".J-op");
		o = "#productMallOptionPanel" + y
	}
	var u = [];
	var l = jm("#mallPrice").html();
	var s = jm("#mallAmount").html();
	var c = jm("#mallWeight").html();
	if(s <= 0) {
		E(z.find("label"));
		z.find("label").attr("isEmptyAmount", "1")
	} else {
		jm.each(k, function(G, I) {
			if(I.oAmount <= 0 && z.length <= 1) {
				var H = z.find("label[data='" + I.t2 + "']");
				E(H);
				jm(H).attr("isEmptyAmount", "1")
			} else {
				if(I.oAmount <= 0) {
					u.push(I.t2)
				}
	...
	jm(z).each(function(G) {
		var H = jm(this);
		H.find("label").click(function() {
			if(jm(this).attr("isEmptyAmount") == "1") {
				return
			}
			var Q = jm(this);
			if(!Q.hasClass("isSelected")) {
				if(Q.parent().find(".optionSelected").length > 0) {
					Q.parent().find(".optionSelected").removeClass("optionSelected isSelected")
				}
				Q.addClass("optionSelected isSelected");
				var O = Q.attr("data");
				H.attr("option_data", O)
			} else {
				Q.parent().find(".optionSelected").removeClass("optionSelected isSelected");
				jm("#" + Q.attr("for")).prop("checked", false);
				Q.parents(".J-op").removeAttr("option_data");
				jm("#mallPrice").html(l);
				jm("#mallAmount").html(s);
			...
			if(z.length > 1 && A) {
				var ai = "";
				var al = 0;
				z.each(function(an, ao) {
					if(jm(z[an]).find(".optionSelected").length <= 0) {
						ai += "x_"
					} else {
						ai += jm(z[an]).find(".optionSelected").attr("data") + "_"
					}
				});
				ai = ai.substring(0, ai.length - 1);
				jm.each(k, function(an, ao) {
					if(F(ai, ao.t2)) {
						if(ao.flag != 0 && !isNaN(ao.oAmount)) {
							al += ao.oAmount
						}
					}
				});
				jm(o + " #mallAmount").text(al)
			}
			var V = f.find(".optionSelected");
			if(B.length > 0) {
				var ah = Q.attr("_indeptPic");
				if(ah != null) {
					B.attr("src", ah)
				}
			}
			if(z.length == V.length) {
				var M = "",
					ab, N, I, R, T;
				jm(V).each(function(an) {
					if(C) {
						M += jm(this).attr("data") + "_"
					} else {
						M += jm(this).attr("data")
					}
				});
				jm.each(k, function(an, ap) {
					var ao = ap.t2;
					if(ap.t == M) {
						ab = ap.oPrice;
						N = ap.oAmount;
				...
				if(x) {
					jm(o + " #limitAmountDiv").text("");
					var J = 0;
					var am = p.optionsCount;
					if(am != null && am.d != null) {
						for(var af = 0; af < am.d.length; af++) {
							if((am.d[af].t + "_") == M) {
								J = am.d[af].c
							}
						}
					...
					var ae = -1;
					if(I > 1 && R != 0) {
						jm(o + " #limitAmountDiv").parent().attr("style", "line-height:1.5rem;");
						jm(o + " #limitAmountDiv").text("( " + LS.minAmount + " : " + I + " ; " + LS.maxAmount + " : " + P + " )");
						aa = I;
						ae = P
					} else {
						if(I == 1 && R != 0) {
							jm(o + " #limitAmountDiv").parent().attr("style", "line-height:1.5rem;");
							jm(o + " #limitAmountDiv").text("( " + LS.maxAmount + " : " + P + " )");
							ae = P
						} else {
							if(I > 1 && R == 0) {
								jm(o + " #limitAmountDiv").parent().attr("style", "line-height:1.5rem;");
								jm(o + " #limitAmountDiv").text("( " + LS.minAmount + " : " + I + " )");
								aa = I
							}
						}
					}
					jm(o + " #productNum").val(aa);
					jm(o + " #productNum").unbind("change");
					jm(o + " #productNum").bind("change", function() {
						var an = jm(o + " #productNum").val();
						var ao = 1;
						if(jm.isInteger(an)) {
							ao = parseInt(an)
						}
						if(ao < aa || ao == aa) {
							ao = aa;
							jm(o + " #g_decrease , .g_decrease").addClass("g_opacity50");
							jm(o + " #g_increase , .g_increase").removeClass("g_opacity50")
						} else {
							if(ae != -1 && (ao > ae || ao == ae)) {
								ao = ae;
								jm(o + " #g_decrease , .g_decrease").removeClass("g_opacity50");
								jm(o + " #g_increase , .g_increase").addClass("g_opacity50")
							} else {
								if(ao > 9999998) {
									ao = 9999999;
									jm(o + " #g_decrease , .g_decrease").removeClass("g_opacity50");
						...
						jm(o + " #productNum").val(ao)
					});
					jm(o + " #productNum").change();
					jm(o + " #g_decrease , .g_decrease").unbind("click.dc2");
					jm(o + " #g_increase , .g_increase").unbind("click.ic2");
					jm(o + " #g_decrease , .g_decrease").bind("click.dc2", function() {
						var an = jm(o + " #productNum").val();
						var ao = 1;
						if(jm.isInteger(an)) {
							ao = parseInt(an)
					...
					jm(o + " #g_increase , .g_increase").bind("click.ic2", function() {
						var an = jm(o + " #productNum").val();
						var ao = 1;
						if(jm.isInteger(an)) {
							ao = parseInt(an)
				...
				if(A) {
					jm(o + " #mallAmount").text(N)
				}
				if(ab == null) {
					Mobi.onlyChangeSalePrice(d, w, a, y)
				} else {
					Mobi.onlyChangeSalePrice(ab, w, a, y)
				}
				if(T != null) {
					jm(o + " #mallWeight").text(T + j)
				}
			}
			if(z.length != V.length && z.length != 1) {
				t()
			}
			if((z.length - 1) <= V.length && z.length != 1) {
				var Z = "";
				z.each(function(an, ao) {
					if(jm(z[an]).find(".isSelected").length <= 0) {
						Z += "x_"
					} else {
						Z += jm(z[an]).find(".isSelected").attr("data") + "_"
					}
				});
				if(Z != "") {
				...
				var S = "";
				z.each(function(an, ap) {
					var ao = jm(z[an]).find(".isSelected");
					if(ao.length <= 0) {
						S = jm(z[an])
					}
				});
				if(S != "") {
					var aj = S.find("label");
					aj.each(function(an, ap) {
						var ao = Z.replace("x", jm(aj[an]).attr("data"));
						if(jm.inArray(ao, u) >= 0) {
							E(aj[an]);
							jm(aj[an]).attr("isEmptyAmount", "1")
						} else {
							h(aj[an])
						}
					})
					...
					z.each(function(an, ap) {
						var ao = jm(z[an]).find("label");
						ao.each(function(at, ar) {
							if(jm(ao[at]).hasClass("isSelected")) {
								return
							}
							var aq = Z.split("_");
							aq[an] = jm(ao[at]).attr("data");
							var au = aq.join("_");
							if(jm.inArray(au, u) >= 0) {
								E(ao[at]);
								jm(ao[at]).attr("isEmptyAmount", "1")
							} else {
								h(ao[at])
							}
						})
			...
			for(var af = 0; af < z.length; af++) {
				var U = z.eq("" + af + "");
				if(U.find(".optionSelected").length > 0) {
					ag.push(U.find(".optionSelected").attr("data"))
				} else {
					break
				}
			}
			var ad = false;
			if(ag.length <= z.length && ag.length > 1) {
				var X = ag.length;
				for(var af = 0; af < X; af++) {
					jm.each(k, function(ao, aq) {
						var ap = aq.t2;
						var an = ap.split("_");
						if(aq.flag == undefined) {
							aq.flag = 1
						}
						if(aq.flag == 1 && arrHasSameBegin(an, ag)) {
							ad = true;
							return false
						}
					});
					if(!ad) {
						var Y = z.eq(ag.length - 1).find("label");
						z.eq(ag.length - 1).removeAttr("option_data");
						Y.removeClass("optionSelected isSelected");
						var W = ag.pop();
						Y.each(function() {
							if(jm(this).attr("data") == W) {
								jm(this).hide()
							}
						})
					} else {
			...
			var ak = z.index(H);
			for(var ac = ak; ac < ag.length; ac++) {
				var K = ag.slice(0, ac + 1);
				ad = false;
				for(var af = K.length; af < z.length; af++) {
					var L = z.eq("" + af + "");
					L.find("label").each(function() {
						var an = jm(this).attr("data");
						ad = false;
						jm.each(k, function(ap, ar) {
							var aq = ar.t2;
							var ao = aq.split("_");
							if(ar.flag == undefined) {
								ar.flag = 1
							}
							if(an == ao[af] && ar.flag == 1 && arrHasSameBegin(ao, K)) {
								ad = true;
								return false
							}
						});
	...
	function E(G) {
		jm(G).css("border-style", "dashed").css("color", "#CDCDCD");
		jm("#" + jm(G).attr("for")).attr("disabled", true)
	}

	function h(G) {
		jm(G).css("border-style", "").css("color", "");
		jm(G).attr("isEmptyAmount", "");
		jm("#" + jm(G).attr("for")).removeAttr("disabled")
	}
...
Mobi.waterFall = function(C, E, D, n, h, m, o, G) {
	var j = jm("#" + E);
	var t = j.parents(".J_formTabContent").length > 0;
	if(t) {
		j.parents(".J_formTabContent").addClass("showTabContent")
	}
	var d = parseInt(jm("#" + E + " .productWaterFall .style8Img").width());
	var l = _htmlFontSize;
	var p = (d * m) / (h * l);
	p = parseFloat(p.toFixed(4));
	var y = (d * n) / (D * l);
	y = parseFloat(y.toFixed(4));
	var F = jm("#" + E + " .paramPadding .paramName").css("font-size");
	if(typeof F != "object") {
		var q = F.substring(0, F.indexOf("p")) / l;
		q = parseFloat(q) + 0.2;
		jm("#" + E + " .productWaterFall .paramPadding .paramName").css("line-height", q + "rem");
		q = parseFloat(q.toFixed(4)) * 2
	} else {
		q = 0
	}
	var v = "paramWrap";
	if(o == v) {
		if(Mobi.getBrowserType() == Mobi.BrowserType.FIREFOX) {
			jm("#" + E + " .productWaterFall .paramPadding .paramName").height(q + "rem")
		} else {
			jm("#" + E + " .productWaterFall .paramPadding .paramName").height(q - 0.08 + "rem")
		}
	} else {
		jm("#" + E + " .productWaterFall .paramPadding .paramName").height(q / 2 + "rem")
	}
	if(C.length == 1) {
		jm("#" + E + " .productWaterFall .style8Img").height(p + "rem");
		jm("#" + E + " .productWaterFall").eq(w).css({
			"vertical-align": "top"
		})
	} else {
		if(C.length == 2) {
			for(var w = 0; w < C.length; w++) {
				if(w % 2 == 0) {
					jm("#" + E + " .productWaterFall .style8Img").eq(w).height(p + "rem");
					jm("#" + E + " .productWaterFall").eq(w).css({
						"vertical-align": "top"
					})
				} else {
					if(w % 2 == 1) {
						var B = 0;
						jm("#" + E + " .productWaterFall .style8Img").eq(w).height(y + "rem");
						jm("#" + E + " .productWaterFall").eq(w).css({
							position: "absolute",
							top: B + "rem"
						});
						var c = parseFloat(jm("#" + E + " .productWaterFall .commonProRight").parent().parent().parent().height());
						var s = parseFloat(jm("#" + E + " .productWaterFall .firstLastPro").parent().parent().parent().height());
						c = c / l;
						s = s / l;
						if(s < c) {
							c = c + 10 / l;
							jm("#" + E + " .productWaterFall .firstLastPro").parent().parent().parent().parent().height(c + "rem")
						}
					}
				}
			}
				...
				if(w == 0) {
					...
					if(u + 1 == C.length) {
						...
						if(C.length % 2 == 1) {
							...
							if(C.length % 2 == 0) {
								var b = parseFloat(jm("#" + E + " .productWaterFall .commonProRight").parent().parent().parent().height());
								b = (b) / l;
								var z = (b + 0.5) * ((w + 1) / 2 - 1);
								jm("#" + E + " .productWaterFall").eq(w).css({
									position: "absolute",
						...
						jm("#" + E + " .productWaterFall .style8Img").eq(w).height(y + "rem");
						var b = parseFloat(jm("#" + E + " .productWaterFall .commonProRight").parent().parent().parent().height());
						var a = parseFloat(jm("#" + E + " .productWaterFall .commonProRight").parent().parent().parent().width());
						b = (b) / l;
						if(w % 2 == 0) {
							jm("#" + E + " .productWaterFall").eq(w).css({
								"vertical-align": "top"
	...
	if(Fai.top._manageMode) {
		var e = $("#" + E),
			I = e.find(".productWaterFall"),
			H = e.attr("_productselect") == "true" ? true : false,
			A, f;
		I.each(function(k, J) {
			A = $(J);
			f = A.attr("topswitch") == "off";
			waterFallImg = A.find(".waterFallImg");
			waterFallImg.wrap("<div class='J_productListImgWrap fk-productListImgWrap'></div>");
			if(f && !H) {
				A.find(".J_productListImgWrap").append("<div class='f-productListTopFlag' >置顶</div>")
			}
		})
	}
	var r = jm("#" + E + " .productWaterFall");
	r.each(function(k, L) {
		var M = jm(L);
		var K = M.attr("productId");
		if(G[K]) {
			var J = G[K];
			if(J.salePromotionOpen) {
				M.find(".waterFallImg").append("<div class='g_bgColor f-productListTopPromotion'>" + J.saleFlagName + "</div>\n")
			}
		}
	});
	if(t) {
		j.parents(".J_formTabContent").removeClass("showTabContent")
	}
};
...
Mobi.productCrossedSlideSecSwipe = function(b, I) {
	...
	isInTab = jm(p).parents(".J_formTabContent").length;
	if(isInTab) {
		jm(p).parents(".J_formTabContent").addClass("f-hideTabModule").removeClass("f-formTabContent")
	}
	var K = l,
	...
	if(k > N) {
		...
		if(Mobi.isPC()) {
			...
			G.off("mousedown.prodItem").on("mousedown.prodItem", ".J_prodItem", function() {
				m = jm(".editLayer");
				n = E.offset().left
			});
			G.off("click.checkMove").on("click.checkMove", function() {
				if(M) {
					d = false
				} else {
					d = true
			...
			G.off("click.prodLink").on("click.prodLink", ".J_prodItem-a", function(O) {
				return d
			})
		} else {
			f = new iScroll(p, {
			...
			jm("body").off("touchmove.prod").on("touchmove.prod", function(O) {
				if(o === false) {
					O.preventDefault()
				}
			});
			...
			E.children().off("touchmove.banIScroll").on("touchmove.banIScroll", function(O) {
				if(o === true) {
					O.stopPropagation()
				}
			})
	...
	if(isInTab) {
		jm(p).parents(".J_formTabContent").removeClass("f-hideTabModule").addClass("f-formTabContent")
	}
};
Mobi.secKillPdSwipe = function(B, z) {
	var F = 0,
		C = Mobi.getPreviewObject(B),
		u = Mobi.getPreviewObject(z),
		d = C,
		A = parseInt(jm(u).width()),
		I = u.querySelectorAll(".J_prodItem"),
		j = _htmlFontSize,
		k = I.length * 0.4 * j + 0.7 * j,
		l = jm(u),
		H = jm("#" + B),
		o = jm("<div></div>"),
		r = 0,
	...
	if(F > A) {
		...
		if(Mobi.isPC()) {
			...
			l.off("mousedown.prod").on("mousedown.prod", c);
			l.off("mousedown.prodItem").on("mousedown.prodItem", ".J_prodItem", function() {
				s = jm(".editLayer");
				h = H.offset().left
			});
			l.off("click.checkMove").on("click.checkMove", function() {
				if(isMove) {
					canClickA = false
				} else {
					canClickA = true
			...
			l.off("click.prodLink").on("click.prodLink", ".J_prodItem-a", function(J) {
				return canClickA
			})
		} else {
			b = new iScroll(u, {
				...
				onBeforeScrollEnd: null,
				onScrollEnd: function() {
					H.find("div.J_pdImg").trigger("horizonScroll")
				}
			});
			l.off("touchstart.prod").on("touchstart.prod", e);
			jm("body").off("touchmove.skProd").on("touchmove.skProd", function(J) {
				if(q === false) {
					J.preventDefault()
				}
			});
			...
			H.children().off("touchmove.banIScroll").on("touchmove.banIScroll", function(J) {
				if(q === true) {
					J.stopPropagation()
				}
			})
	...
	function c(J) {
		...
		r = x = J.pageX;
		p = w = J.pageY;
		l.off("mousemove.prod").on("mousemove.prod", m);
		l.off("mouseup.prod").on("mouseup.prod", G);
		l.off("mouseout.prod").on("mouseout.prod", E);
		J.preventDefault();
		J.stopPropagation()
	}
	...
	function n(M, O, L, N) {
		...
		return J / K > 1 ? true : false
	}

	function f(J) {
		var K = J.touches.length > 0 ? J.touches[0] : J.chengedTouches[0];
		return {
			pageX: K.pageX,
			pageY: K.pageY
		}
	...
	function e(J) {
		...
		r = x = K.pageX;
		p = w = K.pageY;
		l.off("touchmove.prod").on("touchmove.prod", a);
		l.off("touchend.prod").on("touchend.prod", D);
		H.append(o)
	}
	...
	function D(J) {
		o.remove();
		q = r = p = x = w = undefined;
		l.off("touchmove.prod", a);
		l.off("touchend.prod", D)
	}
};
...
Mobi.initMallCartProductNum = function() {
	jm.ajax({
		type: "post",
		...
		success: function(a) {
			...
			if(c.success) {
				if(c.rt == 1) {
					...
					if(b > 0) {
						b = (b > 99) ? "99+" : b;
						jm(".j_mallCartProductNum").text(b);
						jm(".j_mallCartProductNum").show()
					} else {
						jm(".j_mallCartProductNum").hide();
						jm("#mallCartProductNumParent").hide()
					}
				}
			} else {
				jm(".j_mallCartProductNum").hide();
...
Mobi.productNewDetailEventInit = function(u) {
	...
	var f = u.moduleId,
		M = u.productId,
		D = u.mallOptionsOpen;
	pageStyle = u.pageStyle, returnUrl = u.returnUrl || "", isGroupBuy = u.isGroupBuy || false, groupBuyId = u.groupBuyId || 0, groupBuyDetailId = u.groupBuyDetailId || 0, groupPriceStr = u.groupPriceStr || "", isCutPrice = u.isCutPrice || false, cutPriceId = u.cutPriceId || 0, cutPriceDetailId = u.cutPriceDetailId || 0, cpPriceStr = u.cpPriceStr || "";
	isSecKill = u.isSecKill || false;
	secKillId = u.secKillId || 0;
	...
	jm("#productInfoSize").bind("click", function() {
		...
		jm(".productMallBuyPanel .J_productImmediatelyBug").unbind("click").bind("click", function() {
			if(!isSecKill) {
				Mobi.mallBuy(M, 1)
			} else {
		...
		jm(".productMallBuyPanel .J_groupBuyBtn").unbind("click").bind("click", function() {
			Mobi.mallBuy(M, 1, null, null, null, null, {
		...
		jm(".productMallBuyPanel .J_cpButton").unbind("click").bind("click", function() {
			Mobi.logDog(200535, 0);
			Mobi.cutPrice(cutPriceId, M)
		});
		jm(".productMallBuyPanel .serviceBtn").off("click.pp").on("click.pp", function() {
	...
	var z;
	jm("#productNewDetailPanel" + f + " .J_productAddMall").bind("click", function() {
	...
	var o = {
		...
		callback: function() {
			jm("#couponListBottomPanel").bind("click mousedown touchstart", function(P) {
				P.stopPropagation()
			});
			jm("#contentBottomPanel").show();
			jm(".toUseCoupon").bind("click", function(Q) {
...
Mobi.runGroupBuyCountDown = function() {
	var b = [];
	jm.ajax({
		url: "ajax/mall_h.jsp?cmd=getSystemTime",
		success: function(d) {
			d = jm.parseJSON(d);
			if(d.success) {
				var f = d.now || 0;
				...
				var e = f - h;
				jm(".fk_otherGroupBuy .J_gbItem, .fk-groupBuyDetail .J_gbItem, .fk-groupBuyDetail .J_contentBox").each(function(k, r) {
					var q = jm(r).attr("_groupCreateTime");
					var j = jm(r).attr("_groupEndTime");
					q = q.replace(/-/g, "/");
					j = j.replace(/-/g, "/");
					if(!q || !j || q == "null") {
						return
					...
					var l = true;
					if(jm(r).hasClass("J_contentBox")) {
						l = false
					}
					var o = new Date(q);
					var p = new Date(j);
					var m = jm(r).find(".J_gbCountDown");
	...
	function c(f, l, k, j, e) {
		var o = new Date(new Date().getTime() + e);
		var d = f.getTime() - o.getTime();
		var p = parseInt(d / 1000 / 60 / 60 / 24, 10);
		var m = parseInt(d / 1000 / 60 / 60 % 24, 10) + p * 24;
		var h = parseInt(d / 1000 / 60 % 60, 10);
		var n = parseInt(d / 1000 % 60, 10);
		if(p >= 0 || m >= 0 || h >= 0 || n >= 0) {
			k.find(".J_gbCountDown").html(a(m) + ":" + a(h) + ":" + a(n))
		}
		if(p <= 0 && m <= 0 && h <= 0 && n <= 0) {
			clearInterval(b[l]);
			b[l] = null;
...
Mobi.initGroupBuyDetail = function() {
	...
	jm(".icon-groupBuyToIndex").click(function() {
		Fai.top.document.location.href = "/"
	})
};
...
Mobi.mobiPbCommUploadImgHtml = function(h, c, d, f) {
	...
	var j = {
		...
		buttonText: "",
		uploader: "//" + f + '/ajax/commUpsiteimg_h.jsp?cmd="mobiUpload"',
		onUploadStart: function(l) {
	...
	if(Fai.top._manageMode) {
		...
		jQuery("#file_upload_pt_1").attr("disabled", "true").off("click").on("click", function(l) {
...
Mobi.initAjaxTab = function(c) {
	...
	var b = c.pid,
		...
		a = function(o) {
			...
			if(m == 1 && typeof Fai.top._productRecommentData == "string" && Fai.top._productRecommentData != "" && jm("#productSlide" + f).length < 1) {
				jm(l).append(Fai.top._productRecommentData);
				Mobi.productCrossedSlideSec("mProductList" + f, 1, 1, "paramWrap");
				Mobi.productCrossedSlideSecSwipe("productSlideList" + f, "productSlide" + f)
			}
		},
...
Mobi.scrollFadeIn = function(k, c, o) {
	...
	document.addEventListener("touchstart", function(q) {
		n();
		b()
	}, false);
	...
	document.addEventListener("touchend", function(q) {
		n();
		b()
	}, false);
	document.addEventListener("touchcancel", function(q) {
		n();
		b()
	}, false);
	document.addEventListener("scroll", function() {
...
Mobi.getSalePdIdList = function(b) {
	if(b != undefined) {
		var a = b.id || 0;
		jm.ajax({
			url: "ajax/product_h.jsp?cmd=getSalePdIdList&saleId=" + a,
			success: function(d) {
				var c = jm.parseJSON(d);
				if(c.success) {
					Mobi.isFinishGetPdIdList = true;
					if(c.pdIdList) {
						Mobi.salePdIdList = c.pdIdList
					}
				}
			}
		})
...
Mobi.initSalePromotion = function(d, b, c) {
	Mobi.salePromotionDetail.salePromotionParam = d;
	Mobi.salePromotionDetail.styleType = b;
	Mobi.salePromotionDetail.saleReSalePriceHtml = c;
	var a = {
		triggerId: "J_salePromotion",
		pageBg: true,
		pageHtml: Mobi.salePromotionDetailHtml(d),
		direction: "bottom",
		closeWebPage: "#salePromotionPanel",
		callback: function() {
			jm(".saleContainer").bind("click mousedown touchstart", function(f) {
				f.stopPropagation()
			});
			jm(".toCombine").bind("click", function(f) {
				window.location.href = "pr.jsp?salePdIdList=" + jm.toJSON(Mobi.salePdIdList);
				f.stopPropagation()
			})
		}
	};
	Mobi.initWebPage(a);
	Mobi.salePdIdList = [];
	Mobi.getSalePdIdList(d);
	Mobi.isFinishGetPdIdList = false
};
Mobi.salePromotionDetailHtml = function(m) {
	var a = "";
	var l = "";
	var b = m.other.ruleData || {};
	var d = m.saleType;
	var h = b.d;
	var j = b.s;
	var k = false;
	...
	if(d == 2) {
		...
		f.push("<div id='salePromotionPanel' class='salePromotionPanel'>");
		f.push("<div class='saleContainer'>");
		f.push("	<div class='saleTitle'>促销</div>");
		for(var e = 0; e < h.length; e++) {
			f.push("	<div class='saleContentItem'>");
			f.push("	<div class='saleInfo'><span class='saleInfoBg g_bgColor'></span><span class='saleInfoTip g_mainColor'>" + a + "</span></div>");
			var c = jm.format(l, h[e].m, h[e].n);
			f.push("	<span class='productParamterWords'>" + c);
			if(k) {
				f.push("，<span class='toCombine g_mainColor'>去凑单</span></span>")
			}
			f.push("	</div>")
		}
		f.push("</div>");
...
Mobi.showSalePromotionDl = function(m, k, c) {
	...
	if(m.saleType == "1") {
		...
		var j = "";
		if(!jm(".productNewDetailPanel").hasClass("newCountDownProductName")) {
...
Mobi.showSaleTimeCountDown = function(a, c, f, d, e, h, b) {
	jm.ajax({
		url: "ajax/mall_h.jsp?cmd=getSystemTime",
		success: function(r) {
			r = jm.parseJSON(r);
			if(r.success) {
				...
				var k = "";
				var j = LS.salePromotionBegin;
				var n = LS.salePromotionEnd;
				var m = 0;
				Mobi.salePromotionDetail.showType = o;
				...
				Mobi.salePromotionDetail.saleCountTimeInterval = setInterval(function() {
					var w = parseInt(m / (3600 * 24));
					var s = parseInt(m / 3600) - (w * 24);
					var t = parseInt(m / 60) - (w * 24 * 60) - (s * 60);
					var v = parseInt(m % 60);
					if(Fai.top._lcid != 2052 && Fai.top._lcid != 1028) {
						if(s < 10) {
							s = "0" + s
					...
					if(o == 1 && (h || Fai.top._manageMode) && b != 1) {
						jm(".J_showCountDownClass").html("&nbsp;&nbsp;" + jm.format(k, w, s, t, v))
					}
					if(f == 1 && (!h || Fai.top._manageMode || b == 1)) {
						jm(".showSaleTimeClass").html("&nbsp;&nbsp;" + jm.format(k, w, s, t, v))
					}
					if(f == 1 && (h || Fai.top._manageMode) && b != 1) {
						if(w < 10) {
							w = "0" + w
						...
						jm(".J_salePromotionDayNum").html(w);
						jm(".J_salePromotionHourNum").html(s);
						jm(".J_salePromotionMinuteNum").html(t);
						jm(".J_salePromotionSecondNum").html(v)
					}
					if(m < 0) {
						if(o == 1) {
							m = l;
							...
							if(Mobi.salePromotionDetail.salePromotionParam.saleType == "1") {
								var u = jm(".J_salePrePrice").html();
								if(u != null) {
									jm(".J_saleMallPrice").html(u)
								}
							}
...
Mobi.showGbTimeCountDown = function(a, c, d, b) {
	jm.ajax({
		url: "ajax/mall_h.jsp?cmd=getSystemTime",
		success: function(o) {
			o = jm.parseJSON(o);
			if(o.success) {
				...
				var f = "";
				var e = "距拼团活动开始仅剩：{0}天{1}小时{2}分{3}秒";
				var k = "距拼团活动结束仅剩：{0}天{1}小时{2}分{3}秒";
				var j = 0;
				if(l == 1) {
					f = e;
					j = m
				...
				Mobi.gbCountTimeInterval = setInterval(function() {
					var s = parseInt(j / (3600 * 24));
					var p = parseInt(j / 3600) - (s * 24);
					var q = parseInt(j / 60) - (s * 24 * 60) - (p * 60);
					var r = parseInt(j % 60);
					if(Fai.top._lcid != 2052 && Fai.top._lcid != 1028) {
						if(p < 10) {
							p = "0" + p
					...
					if(l == 1 && (d || Fai.top._manageMode) && b != 1) {
						jm(".J_showCountDownClass").html("&nbsp;&nbsp;" + jm.format(f, s, p, q, r))
					}
					if(!d || Fai.top._manageMode || b == 1) {
						jm(".showSaleTimeClass").html("&nbsp;&nbsp;" + jm.format(f, s, p, q, r))
					}
					if((d || Fai.top._manageMode) && b != 1) {
						if(s < 10) {
							s = "0" + s
						...
						jm(".J_salePromotionHourNum").html(p);
						jm(".J_salePromotionMinuteNum").html(q);
...
Mobi.showMallCartScreen = function(e, d, c, b, j) {
	...
	jm("#module" + e + " .J_fk-mallCart").bind("click", function() {
		...
		var p = "";
		var n = {};
		jm.each(Mobi.showProductCacheList, function(r, s) {
			if(s.id == m) {
				l = true;
				k = s.pinfo;
				q = s.showOptionHtml;
	...
	function f(B, n, D, s) {
		...
		jm("#g_increase").unbind("click.ic").bind("click.ic", function() {
			Mobi.decrease2(u)
		});
		jm("#g_decrease").unbind("click.dc").bind("click.dc", function() {
			Mobi.increase2(u)
		});
		...
		jm("#productMallOptionPanel_mcs_" + e + " #productBuy").unbind("click.msc").bind("click.msc", function() {
			Mobi.mallBuy(u, 0, 1, e)
		});
		jm("#productMallOptionPanel_mcs_" + e + " .J_productImmediatelyBug").unbind("click.msc").bind("click.msc", function() {
...
Mobi.showMallCart = function(b, c) {
	jm("#module" + c + " .J_fk-mallCart").bind("click", function() {
	...
	jm("#" + a + " #productBuy").bind("click", function() {
		Mobi.mallBuy(b, 0, 1, c)
	});
	jm("#" + a + " .J_productImmediatelyBug").bind("click", function() {
...
Mobi.bookingBoxInit = function(b, h) {
	...
	if(f) {
		...
		jm.each(b.contentList, function(k, l) {
			if(l.hide) {
				return true
			}
...
Mobi.genBookingInputStr = function(t, c, h) {
	var r = c.moduleId,
		...
		d = t.headline,
		f = t.input;
	b = (t.placeholder == undefined) ? "" : t.placeholder, dateSettingList = [], isIpad = c.isIpad;
...
Mobi.bookingSubmitPanleEventInit = function(a) {
	var b = a.moduleId;
	jm.each(a.contentList, function(j, h) {
		if(h.hide) {
			return true
		}
		var p = h.id,
		...
		if(l == a.Def_ContentList_Type_DATE) {
			...
			if(l == a.Def_ContentList_Type_FILE_UPLOAD) {
				...
				var e = a.isIE;
				fileSize = (h.size > a.fileSizeLimit) ? a.fileSizeLimit : h.size, fileTypeList = [];
				if(!h.isAllFile) {
					var m = (h.dftl || "") + (h.cftl || "");
					var d = m.split(" ");
					jm.each(d, function(s, q) {
						if(q.length > 0) {
							var r = "*." + q + ";";
							fileTypeList.push(r)
						}
	...
	jm("#module" + b + " .siteFormSend").bind("click", function() {
...
Mobi.initCustab = function(b) {
	if(top == window) {
		return
	}
	var a = jm("#module" + b);
	buttonWrap = a.find(".tabUiDiv");
	if(buttonWrap.find("li").length <= 3) {
		return
	}
	jm("#productTabUl").css("width", "100%");
	jm("#productTabUl").css("padding", "0");
	jm("<div class='J_preBtn f-preBtn faisco-icons'></div><div class='J_nextBtn f-nextBtn faisco-icons'></div>").appendTo(buttonWrap);
	buttonWrap.find(".J_preBtn").on("click", function() {
		var d = jm(".tabUiDiv2").offset();
		var e = jm(".tabUiDiv").width();
		var c = jm(".tabUiDiv2").width();
		if(d.left >= 0) {
			return false
		}
		var f = d.left + 80 + "px";
		jm(".tabUiDiv2").css("margin-left", f)
	});
...
Mobi.initOnlineServiceWidth = function() {
	...
	jm("#customerServiceDiv .serviceClass_1").off("click.log").on("click.log", function() {
		Mobi.logDog(200171, 2)
	});
	jm("#customerServiceDiv .serviceClass_5").off("click.log").on("click.log", function() {
		Mobi.logDog(200171, 1)
	});
	jm("#customerServiceDiv .serviceClass_-1").off("click.log").on("click.log", function() {
...
Mobi.showMapService = function(c) {
	...
	jm("#mobiSearchMapFrame").off("load.map").on("load.map", function() {
		Mobi.mobiHideScroll()
	})
};
...
Mobi.unloadForATag = function() {
	if(!jm.os.supportsTouch) {
		jm("#g_body").delegate("#customerServiceDiv a", "click", function(h) {
			var j = h.target || h.srcElement;
			var d = this;
			var f = Fai.top._onlineServiceJson,
				c;
			if(!d) {
				return
			}
			var k = parseInt(d.getAttribute("id").substring(11));
			var b = jm(d).attr("href");
			if(parseInt(k) === 2 || parseInt(k) === 1) {
				if(Fai.top._manageMode) {
					for(var l in f) {
						if(f[l].type == k) {
							c = f[l];
					...
					if(c.baseSetting.jumpCtrl) {
						var a = c.baseSetting.jumpCtrl.jumpType;
						if(a != 1 && a != 2) {
							return
						}
					}
				}
				if(!!c && parseInt(k) == 2 && c.number == "") {
					alert("请输入接受信息的电话");
					Fai.top.Mobi.getPreviewWindow().Mobi.onlineServiceClickCallBack(k, c.typeStr)
				} else {
					if(!!c && k == 1 && !!c.phoneInfo && c.phoneInfo[0].number == "") {
						alert("请输入电话号码");
						Fai.top.Mobi.getPreviewWindow().Mobi.onlineServiceClickCallBack(k, c.typeStr)
					} else {
...
Mobi.bindServiceBtnEvent = function() {
	...
	a.off("click.ser").on("click.ser", function() {
		Mobi.createMallServicePanel()
	});
...
Mobi.mallServiceList = function() {
	...
	jm(".serviceFullBg, .cancleSer").off("click.ser").on("click.ser", function() {
		$(window).off("scroll");
	...
	jm(".serviceListPanel .qqServiceLine").off("click.log").on("click.log", function(l) {
		Mobi.logDog(200171, 1)
	});
	jm(".serviceListPanel .baiduBridgeServiceLine").off("click.log").on("click.log", function(l) {
		Mobi.logDog(200171, 3)
	});
	jm(".serviceListPanel .phoneServiceLine").off("click.log").on("click.log", function(l) {
		Mobi.logDog(200171, 2)
	});
	if(Mobi.isPC()) {
		jm(".phoneServiceLine").off("click.ser").on("click.ser", function() {
			alert("该功能无法在桌面浏览器中启动！");
			return
		})
	}
...
Mobi.showBaiduBridgeV2 = function() {
	...
	jm("ins#newBridge").off("click", "#nb_icon_wrap").on("click", "#nb_icon_wrap", function() {
	...
	if(jm("ins#newBridge").find("#nb_toolbar_wrap").length > 0 && !Fai.top._manageMode) {
		jm("ins#newBridge").find("#nb_toolbar_wrap").hide()
	}
	Fai.top.Mobi.getPreviewWindow().jm("#nb_icon_wrap").click();
	jm("ins#newBridge").off("click", "#nbwlToggle").on("click", "#nbwlToggle", function() {
		jm("ins#newBridge").hide()
	});
	jm("ins#newBridge").off("click", "#nb_nodeboard_close").on("click", "#nb_nodeboard_close", function() {
		jm("ins#newBridge").hide()
	});
	jm("ins#newBridge").off("click", "#sucess_close").on("click", "#sucess_close", function() {
		jm("ins#newBridge").hide()
	})
};
Mobi.showWeChatService = function(c) {
	...
	jm("#g_web").on("touchmove", function(d) {
		d.preventDefault()
	});
...
Mobi.initLocater = function() {
	...
	if(!b) {
		...
		jm(document).unbind("click").bind("click", function(k) {
...
Mobi.initFooterLanguageEvent = function() {
	...
	d.off("click.lang").on("click.lang", function() {
		a.show();
		c.addClass("langPanelUp")
	});
	a.off("click.lang").on("click.lang", function() {
		jm(this).hide();
...
Mobi.initFooterStyle = function(b) {
	...
	linkFontColor = b.gt.c, linkFontSize = b.gt.s;
	if(b.fb.y == 0) {
...
Mobi.MallAjaxErrno = {
	...
	secKillNotOpen: -809,
	secKillCanNotJoinAgain: -801,
	secKillFrequencyAddImme: -811
};
Mobi.returnMallErrorMsg = function(a) {
	...
	switch(parseInt(a)) {
		...
		case Mobi.MallAjaxErrno.notAdded:
			...
			break;
		case Mobi.MallAjaxErrno.secKillNotOpen:
			Mobi.ing("秒杀活动已失效");
			break;
		case Mobi.MallAjaxErrno.secKillCanNotJoinAgain:
			Mobi.ing("不好意思，您已经参与过该秒杀活动");
			break;
		case Mobi.MallAjaxErrno.secKillFrequencyAddImme:
			Mobi.ing("您的操作太频繁了，请稍后再试");
			break;
		default:
			Mobi.ing(LS.systemError);
			location.reload()
...
Mobi.optionsStr = {};
Mobi.mallBuying = {};
Mobi.mallBuy = function(v, s, b, C, a, h, e) {
	e = e || {};
	var t = e.isGroupBuy || false;
	var q = e.groupBuyId || 0;
	var r = e.groupBuyDetailId || 0;
	var k = e.isSecKill || 0;
	var w = k ? (e.secKillId || 0) : 0;
	if(typeof(Mobi.mallBuying[v]) == "undefined") {
		Mobi.mallBuying[v] = false
	}
	var p = false;
	jm.each(Mobi.mallBuying, function(F, j) {
		if(j) {
			p = true;
			return false
		}
	...
	Mobi.mallBuying[v] = true;
	jm("#productNum").change();
	var u = "id=" + v + "&fromDetail=true",
		B = [],
		z = jm(".J-op"),
		o = true;
	if(b == 1) {
		z = jm("#productMallOptionPanel" + v + "[moduleId='" + C + "'] .J-op")
	}
	var d = false;
	if(!!a) {
		u += "&optionList=" + jm.encodeUrl(jm.toJSON(a))
	} else {
		jm(z).each(function() {
			if(d) {
				return
			...
			var I = jm(this),
				F = I.attr("option_name"),
				H = I.attr("data"),
				G = I.attr("type"),
				j = I.attr("option_data");
			if(j) {
				var J = {};
				J.name = H;
				J.value = parseInt(j);
				if(G != null) {
					J.name = H.replace(/^opIndept/, "");
					J.type = parseInt(G)
				}
				B.push(J);
				d = false
			} else {
				Mobi.ing(jm.format(LS.mallCartChoiceItemError, jm.encodeHtml(F)));
				d = true;
				o = false;
				return false
			}
		...
		if(Mobi.optionsStr[D] != "null" && Mobi.optionsStr[D]) {
			var m = Mobi.optionsStr[D].split("_"),
				E = [];
		...
		if(B.length > 0) {
			u += "&optionList=" + jm.encodeUrl(jm.toJSON(B))
		}
	}
	var c = jm("#productNum");
	if(jm("#productMallOptionPanel" + v + " #productNum").length != 0) {
		c = jm("#productMallOptionPanel" + v + " #productNum")
	}
	var l = c.val(),
		n = 1;
	if(jm.isInteger(l)) {
	...
	if(s === 1) {
		...
		if(A.optList == null || A.optList.length == 0 || !A.optList || A.optList == "null") {
			A.optList = jm.toJSON(B)
		}
		f = "addIme";
		u += "&codata=" + jm.toJSON(A)
	}
	jm.ajax({
		type: "post",
			...
			if(I.success) {
				if(s === 1) {
					...
					if(window.self != window.top && /(iPhone|iPad|iPod|safari)/i.test(navigator.userAgent.toLowerCase())) {
						location.href = "mstl.jsp?immeData=" + jm.encodeUrl(jm.toJSON(A)) + H
					} else {
						location.href = "mstl.jsp?imme" + H
					}
				} else {
				...
				if(I.rt == Mobi.MallAjaxErrno.authMemberNotAllow) {
					Mobi.ing(I.data + LS.authLevelBuy)
				} else {
					if(I.rt == Mobi.MallAjaxErrno.login) {
						var F = {};
						F.buyType = s;
						F.productId = v;
						F.entry = b;
						F.moduleId = C;
						F.option = jm.toJSON(B);
						F.count = n;
						F.options = e;
						localStorage.setItem("mallBuyCallBack", jm.toJSON(F));
						var G = "login.jsp?returnUrl=" + jm.encodeUrl(jm.getUrlRoot(location.href)) + "&errno=11";
						location.href = G
					} else {
						Mobi.returnMallErrorMsg(I.rt)
					}
				}
				Mobi.mallBuying[v] = false
			}
...
Mobi.SelfRaisingList = [];
Mobi.initSelfRaisingSelect = function(c) {
	Mobi.SelfRaisingList = c;
	var b = "";
	var a = "";
	jm("#propListTab").delegate(".innerTitle", "click", function() {
		var j = jm("#propListTab").find(".innerTitle");
		var h = jm(this).attr("_mode");
		var e = jm("#shipLine");
		var f = jm("#shipTplLine");
		j.css("border-bottom-style", "unset");
		j.css("color", "#999999");
		jm(this).css("border-bottom-style", "solid");
		jm(this).css("color", "#333333");
		if(h == 1) {
			jm("#totalValue").text(b);
			jm(".orderAssign_line").show();
			jm("#selfRaisingMsg").hide();
			if(e.length != 0) {
				e.show()
			} else {
				if(f.length != 0) {
			...
			if(h == 2) {
				var d = jm("#totalValue").text();
				if(b == "" || (b != d && b != a)) {
					b = d;
					a = Mobi.calculate(b, jm("#shipPrice").attr("value"), 1)
				}
				jm("#totalValue").text(a);
				jm(".orderAssign_line").hide();
				jm("#selfRaisingMsg").show();
				if(e.length != 0) {
					e.hide()
				} else {
					if(f.length != 0) {
	...
	jm("#selfRaisingPointInfo").delegate(".selfRaisingPointPanel", "click", function() {
		var h = jm(".selfRaisingPointName").attr("_pointId");
		var e = jm(this).attr("_pointId");
		var j = "";
		if(h != e) {
			jm("#selfRaisingPointInfo").find(".faisco-icons-right").hide();
			jm("#selfRaisingPointInfo").find(".faisco-icons-shape11").show();
			jm(this).find(".faisco-icons-right").show();
			jm(this).find(".faisco-icons-shape11").hide();
			...
			var d = jm("#selectSelfRaisingPoint").find(".selfRaisingPointName");
			d.attr("_pointId", e);
			d.html(jm.encodeHtml(j));
			jm("#editPointInfo").removeClass("webPagePanelShow")
		}
	})
};
Mobi.initOrderRaisingPanel = function(b, c) {
	jm(".shippingItem").hide();
	jm("#MDPointInfo").click(function() {
		a()
	});
	...
	function a() {
		...
		d.getCurrentPosition(function(l) {
			if(this.getStatus() == BMAP_STATUS_SUCCESS) {
				var f = new BMap.Marker(l.point);
				e.addOverlay(f);
				e.panTo(l.point);
				var j = l.point.lat;
				var h = l.point.lng;
				var k = "http://api.map.baidu.com/direction?";
				k += "origin=" + j + "," + h;
				k += "&destination=name:" + jm.encodeUrl(b) + "|latlng:" + c.lat + "," + c.lng + "";
				k += "&region=北京&mode=driving&output=html";
...
Mobi.initModuleMallAddrInfo = function(A, J, P, u, F) {
	...
	jm("#areaBox").unbind("click").bind("click", function(R) {
	...
	jm("#areaBoxInput").unbind("focus").bind("focus", function() {
		if(jm("#allArea").val() == "cn") {
			p();
			jm("#areaBox1").show();
			jm("#areaBox2").hide();
	...
	jm("#addrInfo").delegate(".addrInfo .info_name", "click", function() {
	...
	var D = J,
		...
		z = "",
		l = true;
	length = D.length, isAddAddrInfoClick = false;
	var k = {};
	jm("#addrInfoList .orderAddAddrInfo").click(function() {
	...
	jm("#addrInfo").delegate(".edit", "click", function() {
	...
	jm("#addrInfo").delegate(".selectedDiv label", "click", function() {
		C = "setDefault";
		var S = 0;
		if(D.length == 1) {
			return
	...
	jm("#g_body").children("#temBox").remove();
	jm("#editCusInfo").delegate(".info_name", "click", function() {
...
Mobi.initModuleMallShipTemplate = function(A, b, C, n, t, p, c) {
	...
	jm("#mallShipTemplate_areaBox").unbind("click").bind("click", function(G) {
	...
	jm("#mallShipTemplate_allArea").change(function() {
		...
		if(jm("#mallShipTemplate_allArea").val() == "cn" || jm("#mallShipTemplate_allArea").val() == "os") {
			...
			jm.each(f, function(J, L) {
				if(v) {
					var K = false,
						I = C.scrl || [];
					jm.each(I, function(N, M) {
						if(M.st === L.type) {
							var O = false;
							if(h) {
				...
				jm.each(L.regionList, function(T, W) {
					var N = false;
					if(h) {
	...
	jm("#mallShipTemplate_areaBoxInput").unbind("click").bind("click", function() {
		if(jm("#mallShipTemplate_allArea").val() == "cn") {
			E();
	...
	jm("#mallShipTemplate_province_box .group_item").bind("click", function() {
		jm("#mallShipTemplate_areaBox1 .city_content").remove();
		jm("#mallShipTemplate_areaBox1 .county_content").remove();
		...
		jm.each(f, function(I, J) {
			Mobi.changeShipTplPrice(J)
		});
		Mobi.mallStlCalcTotal()
	});
	jm("#mallShipTemplate_city_box").delegate(".group_item", "click", function() {
		Mobi.mallStlCalcTotal();
		...
		jm.each(f, function(M, O) {
			if(v) {
				var N = false,
					L = C.scrl || [];
				jm.each(L, function(Q, P) {
					if(P.st === O.type) {
						var R = false;
						if(h) {
			...
			jm.each(O.regionList, function(W, Z) {
				var Q = false;
				if(h) {
	...
	jm("#mallShipTemplate_county_box").delegate(".group_item", "click", function() {
		jm("#mallShipTemplate_areaBox1 .street_content").remove();
	...
	jm("#mallShipTemplate_street_box").delegate(".group_item", "click", function() {
		var F = jm(this).attr("cname");
		var H = jm(this).attr("cid");
	...
	jm("#mallShipTemplate_sec_box").delegate(".group_item", "click", function() {
		if(jm("#mallShipTemplate_allArea").val() == "" || jm("#mallShipTemplate_allArea").val() == "cn" || jm("#mallShipTemplate_allArea").val() == "os") {
			h = true
		} else {
			h = false
	...
	jm("#mallShipTemplate_thd_box").delegate(".group_item", "click", function() {
		if(jm("#mallShipTemplate_allArea").val() == "" || jm("#mallShipTemplate_allArea").val() == "cn" || jm("#mallShipTemplate_allArea").val() == "os") {
			h = true
		} else {
			h = false
		...
		jm.each(f, function(L, N) {
			if(v) {
				var M = false,
					K = C.scrl || [];
				jm.each(K, function(P, O) {
					if(O.st === N.type) {
						var Q = false;
						if(h) {
			...
			jm.each(N.regionList, function(V, Y) {
				var P = false;
				if(h) {
...
Mobi.initUseCoupon = (function(j) {
	...
	function b(m) {
		...
		k = this;
		l.couponList = m;
		l.couponColorList = ["red", "orange", "yellow", "green", "blue", "pink", "purple"], d(m);
		c();
		h()
	}
	...
	function c() {
		...
		Mobi.onTouchAndClickEvent("#editCouponPanel .fk-coupon-opera .g_button", function(m) {
	...
	function f() {
		...
		jm.each(l.couponList, function(p, o) {
	...
	function a() {
		var o = [];
		var m = [];
		jm.each(l.couponList, function(p, q) {
	...
	function h() {
		var m = {
			...
			pageBg: true,
			callback: function() {
				jm(".couponListTitle,#couponUseListContent").bind("click mousedown touchstart", function(n) {
					n.stopPropagation()
				});
				jm("#couponListItems").css("overflow", "auto");
				jm("#couponLine").bind("click", function(n) {
...
Mobi.setUseIntegral = function() {
	var a = Fai.top._choiceCurrencyVal;
	if(jm.os.supportsTouch) {
		jm("#editUseItgPanel .g_button").off("touchend").on("touchend", function() {
			setTimeout(function() {
		...
		jm("#editUseItgPanel .g_button").off("mousedown").on("mousedown", function() {
			if(Mobi.initPresentIpt()) {
				jm("#editUseItgPanel .g_close").click()
			}
		})
	...
	if(jm.os.supportsTouch) {
		jm("#editUseItgPanel .notUseBtn").off("touchend").on("touchend", function() {
			setTimeout(function() {
		...
		jm("#editUseItgPanel .notUseBtn").off("mousedown").on("mousedown", function() {
			jm("#offsetMoney").attr("_offsetmoney", "0");
			jm("#useItg").val("0");
			Mobi.initPresentIpt();
			Mobi.mallStlCalcTotal();
...
Mobi.editInfoSave = function(c) {
	...
	if(jm(".assignItem_line").length < 1) {
		...
		if(Fai.top._templateFrameId >= 2000 && Fai.top._templateFrameId < 3000) {
			jm("#assignItemline").off("click.mall").on("click.mall", function() {
				jm("#linetitle1").click()
			})
		}
	} else {
...
Mobi.mallSubmit = function(C, B, d, N, x, r, l, o, y, u) {
	if(!d && C === 0) {
	...
	var a = jm(".orderSettle"),
		...
		H = "",
		b = {};
	if(jm(".orderAssign_line").length != 0 && jm(".orderAssign_line").css("display") != "none") {
		a.find(".propItemValue").each(function() {
			var W = jm(this),
				R = W.val(),
				Q = W.attr("id");
			required = W.attr("_required") == 1 || W.attr("_required") == "true";
			if(required && !R && !z) {
				H = LS.mallStlSubmitInput + W.attr("_prop");
				Mobi.ing(H);
				jm("#linetitle1").click();
				jm("#" + Q).trigger("focus");
				z = true;
			...
			var Y = W.attr("_field");
			var P = W.attr("proptype");
			if(R && R.length > 0) {
				if(P == 8 || P == 10) {
					if(!jm.isNationMobile(R)) {
						jm("#" + Q).trigger("focus");
						H = LS.mobileNumRegular;
						Mobi.ing(H);
						z = true;
						return false
					}
				...
				if(P == 9) {
					...
					if(R.length > 0) {
						var Z = {};
						Z.s = R;
						var V = Q.replace("input", "") + "_endinput";
						var U = a.find("#" + V);
						if(U.length > 0) {
							var T = U.val();
							if(T == "") {
			...
			if(typeof Y !== "undefined") {
				if(Y !== "addrDetail" && Y !== "addrInfo_street") {
					M[Y] = R;
					b[Y] = R;
					if(Y == "mobile") {
						var S = jm("#mobileCt").val();
						if(R.length <= 0) {
							S = ""
						}
						M.mobileCt = S
		...
		var D = jm("#areaBoxInput");
		if(M.addr == null && D.length == 1) {
			var p = "";
			if(Fai.top._lcid == 2052 || Fai.top.lcid == 1028) {
				p = D.val().replace(/-/g, "") + jm("#addrInfo_street").val()
			} else {
				p = D.val().replace(/-/g, " ") + " " + jm("#addrInfo_street").val()
			}
			M.addr = p
		...
		var F = jm("#shipValue");
		if(jm("#mallShipTemplate_province").length == 1 && F.length == 1) {
			var s = parseInt(F.attr("provinceCode"));
			var E = parseInt(F.attr("cityCode"));
			var v = parseInt(F.attr("countyCode"));
			var n = parseInt(F.attr("streetCode"));
			var j = (jm("#mallShipTemplate_areaBoxInput").attr("isCus") == 1);
			if(!s || isNaN(s)) {
			...
			var m = {
				type: parseInt(F.attr("shipType")),
				templateId: parseInt(F.attr("templateId")),
				provinceCode: s,
				cityCode: E,
				countyCode: v,
				streetCode: n,
			...
			M.shipType = parseInt(jm("#shipValue").attr("shipType"))
		}
		M.isTakingPoint = false
	} else {
		if(jm("#selfRaisingMsg").length != 0 && jm("#selfRaisingMsg").css("display") != "none") {
			var I = jm("#selfRaisingMsgName").val();
			var c = jm("#selfRaisingMsgPhone").val();
			var f = jm("#selfRaisingPoint").attr("_pointId");
			var t = Mobi.SelfRaisingList;
			if(I != "") {
				M.name = I
			} else {
				Mobi.ing("请输入提货人姓名。");
			...
			if(c != "") {
				if(jm.isPhone(c) && c.length == 11) {
					M.phone = c
				} else {
					Mobi.ing("请输入11位正确的手机号。");
					return
				}
			} else {
				Mobi.ing("请输入自提人手机号码。");
			...
			if(f > 0) {
				for(var K = 0; K < t.length; K++) {
					if(t[K].id == f) {
						M.addrInfo = '{"srp":' + jm.toJSON(jm.toJSON(t[K])) + "}";
						break
					}
				}
			} else {
				Mobi.ing("自提点信息错误，请联系管理员。");
				return
			}
			M.isTakingPoint = true
		}
	}
	if(Mobi.orderLvMessageOpen) {
		var L = jm("#orderLeveaMsgIn").val();
		if(L.length > Mobi.orderLvMessageMaxNum) {
			L = L.substring(0, Mobi.orderLvMessageMaxNum)
		}
		M.msg = L
	} else {
		M.msg = ""
	...
	var G = jm("#onlineBankType");
	var k = 0;
	if(G.length == 1) {
		k = parseInt(G.attr("_paymode"));
		if(k < 3) {
			M.payMode = k
		}
	}
	if((G.length == 1) && (k > 2)) {
		k = parseInt(G.attr("_paymode"));
		if(k != 9 && k != 8 && k != 7) {
			M.payBankType = parseInt(G.attr("_type"))
		}
		M.payMode = parseInt(G.attr("_paymode"))
	}
	if(isNaN(k)) {
	...
	if(e.length > 0) {
		var O = e.val();
		if(jm.isInteger(O)) {
			var w = parseInt(e.attr("_maxUse"));
			var J = parseInt(e.attr("_currentitg"));
			var h = e.attr("_itegName");
			if(O > J) {
				Mobi.ing(jm.format(LS.integral_notOverCurrent, jm.encodeHtml(h), jm.encodeHtml(h)));
				e.focus();
				return
			}
			if(O > w) {
				Mobi.ing(jm.format(LS.integral_notOver, jm.encodeHtml(h), w));
				e.focus();
				return
			}
			if(O < 0) {
	...
	if(Mobi.selectCoupon && "id" in Mobi.selectCoupon && Mobi.selectCoupon.id) {
		M.cdId = Number(Mobi.selectCoupon.id)
	}
	Mobi.ing(LS.mallStlSubmitting, -1);
	var q = jm("#onlineBankType").attr("_paymode");
	urlArg = "orderId=" + C;
	if(d) {
		if(window.self != window.top && /(iPhone|iPad|iPod|safari)/i.test(navigator.userAgent.toLowerCase())) {
			urlArg = "imme&immeData=" + jm.encodeUrl(jm.toJSON(r))
		} else {
			urlArg = "imme"
		}
	}
	...
	if(!d && N.length > 0) {
		jm.ajax({
			type: "post",
			url: "ajax/order_h.jsp?cmd=delItem&orderId=" + C,
			data: "itemIds=[" + N + "]",
			error: function() {
				Mobi.ing(LS.mallCartUpdateError)
			},
			success: function(P) {
				var P = jm.parseJSON(P);
				if(P.success) {
					Mobi.ajaxSettleOrder(urlArg, M, x, d, q, C, l, o, u)
				} else {
					if(P.rt == -9) {
...
Mobi.ajaxSettleOrder = function(f, c, a, h, e, b, k, d, j) {
	...
	jm.ajax({
		...
		data: "data=" + jm.encodeUrl(jm.toJSON(c)) + "&groupBuyOptions=" + jm.encodeUrl(jm.toJSON(k)) + "&cutPriceOptions=" + jm.encodeUrl(jm.toJSON(d)) + "&options=" + jm.encodeUrl(jm.toJSON(j)),
		error: function() {
			Mobi.mallSubmiting = false;
			Mobi.ing(LS.mallStlSubmitError)
		},
		success: function(l) {
			...
			if(l.success) {
				jm.ajax({
					...
					data: "data=" + jm.encodeUrl(jm.toJSON(a)),
					error: function() {
						Mobi.mallSubmiting = false;
					...
					success: function(n) {
						if(e == "10" || e == "11") {
							var o = Fai.top._lcid ? "&lcid=" + Fai.top._lcid : "";
							window.location.href = "/wxPay/mdetail.jsp?id=" + (h ? l.oid : b) + o + "&wxFirst=true#firstSubmit"
						} else {
							if(e == "1" || e == "2") {
								window.location.href = "mdetail.jsp?id=" + (h ? l.oid : b) + "&suc=true"
							} else {
								window.location.href = "mdetail.jsp?id=" + (h ? l.oid : b) + "#firstSubmit"
							}
						}
					}
				})
				...
				if(l.rt == -3) {
					...
					if(l.rt == -9) {
						...
						if(l.rt == Mobi.MallAjaxErrno.mallAmountZero) {
							...
							if(l.rt == -23) {
								...
								if(l.rt == Mobi.MallAjaxErrno.outOfMallAmount) {
									...
									if(l.rt == Mobi.MallAjaxErrno.OutOfAllowAmount) {
										Mobi.ing(jm.format(LS.allowAmountOverNameList, l.productsName))
									} else {
										if(l.rt == Mobi.MallAjaxErrno.couponOverTime) {
											Mobi.ing(LS.couponOverTime)
										} else {
											if(l.rt == Mobi.MallAjaxErrno.couponUnavail) {
												Mobi.ing(LS.couponUnavail)
											} else {
												if(l.rt == Mobi.MallAjaxErrno.couponNotFound) {
													Mobi.ing(LS.couponNotFound)
												} else {
													if(l.rt <= -600 && l.rt >= -609) {
														var m;
														if(l.rt == -608) {
															m = {
																textClass: "goToPay",
																...
																callback: function() {
																	k.groupBuyDetailId = 0;
																	Mobi.ajaxSettleOrder(f, c, a, h, e, b, k)
																}
															};
															Mobi.prompt(m)
														} else {
														...
														if(l.rt <= -800 && l.rt >= -850) {
															if(l.rt == -801) {
																Mobi.ing("不好意思，您已经参与过该秒杀活动")
															} else {
																if(l.rt == -809) {
																	Mobi.ing("不好意思，该秒杀活动已失效")
																} else {
																	if(l.rt == -810) {
																		Mobi.ing("不好意思，该产品已售罄")
																	} else {
																		Mobi.ing("秒杀失败，请稍后再试")
																	}
																}
...
Mobi.callPayAPI = function(j) {
	...
	try {
		...
		function c() {
			...
			WeixinJSBridge.invoke("getBrandWCPayRequest", e, function(m) {
...
Mobi.setFlowDivSwipeLis = function() {
	...
	jm("#J-flowSwipeDiv").bind("mousedown", function(f) {
		...
		jm("#J-flowSwipeDiv").bind("mousemove", function(h) {}).bind("mouseup", function(j) {
...
Mobi.initFlowButton = function(b) {
	var a = jm("#module" + b);
	a.find(".J-flowView").unbind().bind("click", function() {
		var d = jm(this);
		var c = d.attr("_oid");
...
Mobi.initPayOrderImmediately = function(b) {
	var a = jm("#module" + b);
	a.find(".j-payOrderImmediately").unbind().bind("click", function() {
		var d = jm(this);
		var c = d.attr("_oid");
		Mobi.mallSettle(c)
	})
...
Mobi.initOrderSureButton = function(b) {
	var a = jm("#module" + b);
	a.find(".J-orderSureRpt").unbind().bind("click", function() {
		var h = jm(this),
			e = h.parent().parent(),
			d = h.attr("_oid");
		var c = function(k, j) {
...
Mobi.initOrderCancelButton = function(b) {
	var a = jm("#module" + b);
	a.find(".J-orderCancel").unbind().bind("click", function() {
		var h = jm(this),
			e = h.parent().parent(),
			d = h.attr("_oid");
		var c = function(k, j) {
...
Mobi.initOrderDeleteButton = function(b) {
	var a = jm("#module" + b);
	a.find(".J-orderDelete").unbind().bind("click", function() {
		var f = jm(this),
			d = f.attr("_oid");
		var c = function(h) {
			jm.ajax({
...
Mobi.initOrderPopUpPanel = function() {
	...
	a = {
		triggerId: "selectSelfRaisingPoint",
		panelId: "editPointInfo",
		callback: function() {
			c("editPointInfo", "editAddr")
		},
		closeWebPage: "#editPointInfo .g_close"
	};
	Mobi.initWebPage(a);
	a = {
		triggerId: "MDSelfRaisingPointInfo",
		panelId: "showPointInfo",
		callback: function() {
			c("showPointInfo", "editAddr")
		},
		closeWebPage: "#showPointInfo .g_close"
	};
	Mobi.initWebPage(a);
	var b = {
		triggerId: "shipLine",
	...
	function c(e, j) {
		if(e == "editCusInfo" || e == "editAddrInfo") {
			if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
				jm("#" + e).find("input,select,textarea").bind("blur", function() {
					document.querySelector("body").scrollTop = 0
				});
				jm("#" + e).find("input,select,textarea").bind("focus", function() {
				...
				jm("#areaBox").bind("touchmove", function(l) {
					l.preventDefault()
				});
				jm("#areaBox .J_areaHead").bind("touchmove", function(l) {
					l.preventDefault()
				});
				jm("#areaBox1").bind("touchmove", function(l) {
					l.stopPropagation()
				});
				jm("#areaBox2").bind("touchmove", function(l) {
					l.stopPropagation()
				})
			}
		}
...
Mobi.initCommentBtn = function(d, a) {
	var c = jm("#module" + d),
		b = jm("#g_web");
	c.find(".J-comment").unbind().bind("click", function() {
	...
	_initItemCommentPanel = function(o, w, h, r, t) {
		...
		if(typeof(m) != "undefined" && m != null) {
			...
			if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
				jm(".J-ct-area").bind("focus", function() {
...
Mobi.mallProductDetailFixedBar = function() {
	if(Fai.top._templateFrameId >= 2000 && Fai.top._templateFrameId < 3000) {
		...
		if(Fai.top._manageMode && jm("#serviceBtnOuter").length > 0) {
			jm("#fixedAreaBox").find(".productMallShop").remove();
...
Mobi.initRefundDetail = function(b, h, d, f) {
	...
	jm(".J_delBtn").on("click.delRefundIcon", function() {
...
Mobi.refundPicUploadCallBack = function(b) {
	...
	jm(".J_delBtn").off("click.delRefundIcon").on("click.delRefundIcon", function() {
		jm(this).parent().remove();
		jm("#uploadRefundPicBtn").css("display", "inline-block")
	})
};
...
Mobi.initNewGuestGiftInfo = function(a) {
	jm.ajax({
		type: "post",
		url: "ajax/newGuestGift_h.jsp?cmd=loadModuleDom",
		data: "printerOptions=" + jm.encodeUrl(jm.toJSON(a)),
		error: function() {
		...
		success: function(b) {
			var b = jm.parseJSON(b);
			if(b.success) {
				var d = jm.parseJSON(b.rtInfo);
				jm("#J_newGuestGiftBtn").children().remove();
				jm("#J_newGuestGiftBtn").append(d.dom.btn);
				jm("#J_newGuestGiftPage").children().remove();
				jm("#J_newGuestGiftPage").append(d.dom.page);
				jm(".J_showGiftPage").bind("click", function() {
					jm(".newGuestGiftSreen").show();
					jm(".newGuestGiftContainer").show()
				});
				jm(".J_hiddenGiftPage").bind("click", function() {
					jm(".newGuestGiftSreen").hide();
					jm(".newGuestGiftContainer").hide()
				});
				jm(".J_receiveBtn").bind("click", function() {
					if(Fai.top._manageMode == true) {
						var f = [];
						f.push('<div class="mobiCouponMsgPanl2">');
						f.push('<div class="mobiCouponMsg">');
						f.push('<div class="fTop">领取失败</div>');
						f.push('<div class="fBottom">当前为管理状态!</div>');
						f.push("</div>");
						f.push("</div>");
				...
				var c = jm(".newGuestGiftContainer .giftItem").length || 0;
				if(c == 1) {
				...
				var e = jm(".newGuestGiftContainer .giftItem");
				e.each(function() {
					jm(this).bind("click", function() {
						var f = jm(this).attr("couponid");
						Fai.top.location.href = "pr.jsp?couponId=" + f
					})
				})
			}
		}
...
Mobi.initCutPriceDetailCD = function(a, c, b) {
	...
	jm(".icon-cutPriceToIndex").bind("click", function() {
		Fai.top.document.location.href = "/"
	})
};
Mobi.marketingPdInterval = null;
Mobi.initMarketPdDetail = function(c, a, f, e, k, h) {
	window.onload = function() {
		jm(".showSaleTimeImg").css("display", "block")
	};
	if(c != 0) {
		jm(".J_salePromotionNewMessage").html(LS.salePromotionNewEndMessage);
		if(h == "secKill") {
			jm(".J_salePromotionNewMessage").html("距秒杀结束还有：")
		}
	} else {
		if(h == "secKill") {
			jm(".J_salePromotionNewMessage").html("距秒杀开始还有：")
		}
	}
	if((e || Fai.top._manageMode) && k != 1) {
		jm(".J_salePromotionDay").html(LS.salePromotionDay);
		jm(".J_salePromotionHour").html(LS.salePromotionHour);
		jm(".J_salePromotionMinute").html(LS.salePromotionMinute);
		jm(".J_salePromotionSecond").html(LS.salePromotionSecond)
	...
	var d = null;
	if(Mobi.marketingPdInterval) {
		clearInterval(Mobi.marketingPdInterval)
	}
	Mobi.marketingPdInterval = setInterval(function() {
		var n = new Date().getTime();
		var u = c == 0 ? a - n : f - n;
		var o = Mobi.translateDiffTime(u);
		var q = c == 0 ? "开始" : "结束";
		var r = "距" + b + "活动" + q + l + o;
		if(c == 0 && (e || Fai.top._manageMode) && k != 1) {
			jm(".J_showCountDownClass").html("&nbsp;&nbsp;" + r)
		}
		if(!e || Fai.top._manageMode || k == 1) {
			jm(".showSaleTimeClass").html("&nbsp;&nbsp;" + r)
		}
		if((e || Fai.top._manageMode) && k != 1) {
			var p = o.split(/[^0-9]/ig);
			var v = p[0];
			var s = p[1];
			var m = p[2];
			var t = p[3];
			...
			if(h == "secKill") {
				if(v > 0) {
					jm(".J_salePromotionDay").html("天");
					jm(".J_salePromotionHour").html(":");
					jm(".J_salePromotionMinute").html(":");
					jm(".J_salePromotionSecond").html("");
					jm(".J_salePromotionDayNum").html(v);
					jm(".J_salePromotionHourNum").html(s);
					jm(".J_salePromotionMinuteNum").html(m);
					jm(".J_salePromotionSecondNum").html(t);
					if(d) {
						clearInterval(d)
					}
				} else {
					jm(".J_salePromotionDay").html(":");
					jm(".J_salePromotionHour").html(":");
					jm(".J_salePromotionMinute").html(".");
					jm(".J_salePromotionSecond").html("");
					jm(".J_salePromotionDayNum").html(s);
					jm(".J_salePromotionHourNum").html(m);
					jm(".J_salePromotionMinuteNum").html(t);
					if(!jm(".showNewThemeSaleTimeClass").hasClass("showMillisTime")) {
						jm(".showNewThemeSaleTimeClass").addClass("showMillisTime")
					}
					j()
				}
			} else {
	...
	var j = function() {
		var n = [9, 8, 7, 6, 5, 4, 3, 2, 1, 0];
		var m = 0;
		d = setInterval(function() {
			jm(".J_salePromotionSecondNum").html(n[m]);
			m++;
			if(m >= 10) {
				m = 0
			}
...
Mobi.secKillModuleInterVal = null;
Mobi.initSecKillCountDown = function(d, f, a) {
	var c = "";
	var b = "";
	if(f == 1 || f == 3) {
		c = "还有%s开抢";
	...
	if(Mobi.secKillModuleInterVal != null) {
		clearInterval(Mobi.secKillModuleInterVal)
	}
	Mobi.secKillModuleInterVal = setInterval(function() {
		var h = new Date().getTime();
		var j = jm("#module" + d).find(".J_countTime");
		j.each(function(l, q) {
			var o = jm(q).attr("startTime");
			var k = jm(q).attr("endTime");
			var p = o - h > 0 ? o - h : k - h;
			p -= e;
			if(p >= 3600000 * 24) {
				return
			}
			var m = Mobi.translateDiffTime(p, [":", ":", ":", ""], true, true);
			if(o - h > 0) {
				m = c.replace("%s", m);
				if(!jm(q).hasClass("countTimeBg")) {
					jm(q).addClass("countTimeBg")
				}
			} else {
				if(k - h > 0) {
					m = b.replace("%s", m);
...
Mobi.translateDiffTime = function(d, c, a, b) {
	if(!c || c.length != 4) {
		c = ["天", "时", "分", "秒"]
	}
	var f = [
	...
	var e = "";
	jm.each(f, function(j, k) {
		var h = Math.floor(d / k[1]);
		if(j == 0 && b && h == 0) {
			return
		}
		if((h + "").length == 1 && a) {
...
Mobi.cutPriceImmeBuy = function(k, d, l, h) {
	...
	var e = false;
	jm.each(Mobi.mallBuying, function(p, o) {
		if(o) {
			e = true;
			return false
		}
...
Mobi.initNewsEvent = function(c, o, l, n) {
	...
	if(m > 0) {
		jm("#postComment").on("click", function() {
			...
			if(jm.os.android) {
				a.hide();
				Mobi.bindTouchAndClickEvent(jm("#userCommentPanel .userCommentGoBack"), function() {
					a.show()
				})
			}
		})
...
Mobi.initModuleCardGraphicNewsList = function(t) {
	...
	if(r < x) {
		if(Mobi.isPC()) {
			...
			E.off("touchmove.newlist" + h).on("touchmove.newlist" + h, function(P) {
				if(m === false) {
					P.preventDefault()
				}
			});
			G.off("touchmove.newlist").on("touchmove.newlist", function(P) {
				if(m === true) {
					P.stopPropagation()
				}
			});
...
Mobi.showVoteResult = function(c, a) {
	...
	jm("#fullScreenDivCotainer").append("<div id='voteResult'  class=' webBackground webContainerBox voteResultDiv moduleContent fk-body-bg' style='overflow-y:auto; position: fixed !important;'></div>");
	jm.ajax({
		type: "post",
...
Mobi.popupSearch = function(b) {
	...
	if(typeof b.hotTopic === "string" && b.hotTopic) {
		b.hotTopic = JSON.parse(b.hotTopic)
	}
	var j;
	if(b.searchProduct) {
	...
	e.push('			<form><input id="searchInput" class="popupSearchInput" type="search" placeholder="' + Mobi.encodeHtml(b.searchTip) + '"></form>');
	...
	if(b && b.hotTopic && b.hotTopic.length) {
		...
		for(var d = 0, a = b.hotTopic.length; d < a; d++) {
			e.push('<span class="topicItem">' + Mobi.encodeHtml(b.hotTopic[d]) + "</span>")
		}
		e.push("	</div>")
	}
	if(b.historySearch && j) {
		...
		if(j) {
			for(var d = 0, a = j.length; d < a; d++) {
				e.push('<span class="topicItem">' + Mobi.encodeHtml(j[d]) + "</span>")
			}
		}
		e.push("</div>");
...
Mobi.searchInMobi = function(e, d) {
	...
	var h = e;
	var a = d.nSL;
	var b = "searchResult.jsp?searchword=" + jm.encodeUrl(jm.trim(h)) + "&nSL=" + a + "&searchModuleId=" + f;
...
(function(c, a, d) {
	...
	a.ExposureQueue = {
		add: function(f) {
			var e = (a.ExposureQueue.indexOf(f.name) === -1);
			if(e) {
				b.push(f)
			}
		},
		remove: function(f) {
			var e = a.ExposureQueue.indexOf(f);
			if(e > -1) {
				b.splice(e, 1)
			}
		},
		...
		init: function() {
			a.ExposureQueue.listen()
		},
		listen: function() {
			var j = c(window);
			var h = "resize.exposureList";
			var e = "scroll.exposureList";
			var f = a.throttle(a.ExposureQueue.exposure);
			a.ExposureQueue.exposure();
			j.off(h).on(h, f).off(e).on(e, f)
		},
		exposure: function() {
			b.forEach(function(e) {
				if(jm.isFunction(e.onTrigger)) {
					e.onTrigger()
				}
			})
		}
	...
	c(function() {
		a.ExposureQueue.init()
	})
}(jm, Mobi));
(function(b, a, c) {
	a.bindTabEvent = function(l, e, h) {
		var d = b(l);
		if(!a.isPC()) {
			var f = "touchstart." + e;
			var k = "touchmove." + e;
			var m = "touchend." + e;
			d.off(f).on(f, function() {
				var n = false;
				d.off(k).on(k, function() {
					n = true
				});
				d.off(m).on(m, function() {
					d.off(m).off(k);
					if(!n) {
...
(function(b, a, c) {
	a.manageFaiscoAd = function(d) {
		var f = (Fai.top._manageMode !== true);
		var h = (Fai.top._isFreeVer === true);
		var e = f && h;
		if(!e) {
			return
		}
	...
	a.initCornerAd = function(j) {
		var d = b("#mobiCornerAd");
		var l = (d.length > 0);
		if(l) {
			d.show();
			n(d);
		...
		function n(p) {
			var o = p.find(".J_link");
			var q = "//" + Fai.top._mjzHomeHost + "/proTest.html?_ta=2527";
			o.attr("href", q)
		}
		...
		function e() {
			a.ExposureQueue.add({
				name: "freeAdAbTest",
				onTrigger: function() {
					var o = b("#mobiCornerAd");
					var r = b(window);
					if(a.isInViewArea(r, o)) {
						var q = p();
						if(q !== void 0) {
							a.logDog(200625, q)
						}
						a.ExposureQueue.remove(this.name)
					}
		...
		function m(r) {
			var p = b("#webCustomerServiceBox");
			var o = p.length > 0;
			var q = (p.css("display") !== "none");
			if(q) {
				r.addClass("fk_mobiCornerAd_hasBottomMenu")
			}
		}
		...
		function h() {
			...
			if(s) {
				var r = b(window);
				var p = "resize.adjustAdPos";
				var q = function() {
					var t = b(".webRight").width();
					b("#mobiCornerAd").css({
						right: t + "px"
					})
				};
		...
		function k(p) {
			var o = p.find(".J_close");
			a.bindTabEvent(o, "close", function(q) {
				p.hide();
				q.stopPropagation()
			})
		}
		...
		function f(p) {
			if(a.isWechat()) {
				...
				var o = p.find(".J_link");
				o.removeAttr("href");
				a.bindTabEvent(o, "callWeChat", function() {
					a.WeChatAd.show()
				})
			}
		}
...
(function(b, a, c) {
	a.WeChatAd = {
		...
		init: function() {
			...
			a.WeChatAd.$dom = d;
			a.WeChatAd.bindEvent();

			function f() {
				return ["<div class='popupWXQrCode' id='popupWXQrCode' >", "<div class='wxQrBg J_close'></div>", "<img class='J_bigPic bgPic' src='" + Fai.top._resRoot + "/image/wx/wxQrCodeBack.png'/>", "<div class='wxQrTip'>免费建站</div>", "<img class='wxQrCodePic' src='" + Fai.top._resRoot + "/image/mobiFooterWechatQrCode.gif'/>", "</div>"].join("")
			}
		},
		show: function() {
			a.WeChatAd.$dom.show()
		...
		bindEvent: function() {
			var d = a.WeChatAd.$dom.find(".J_close");
			a.bindTabEvent(d, "close", function() {
				a.WeChatAd.hide()
			});
			a.WeChatAd.listenLongPress()
		},
		listenLongPress: function() {
			var d = a.WeChatAd.$dom.find(".J_bigPic");
			var k = "touchstart.longPress";
			var j = 500;
			var o = "touchend.longPress";
			var h = null;
			var l = null;
			d.off(k).on(k, n);
			d.off(o).on(o, e);
...
Mobi.titlePositionRefreshByLogo = function() {
	...
	logoWidth = parseInt(l.width()), logoMarginLeft = parseInt(l.css("margin-left").replace("px", "")), logoMarginRight = parseInt(l.css("margin-right").replace("px", "")), headerPaddingLeft = parseInt(j.css("padding-left").replace("px", "")), headerPaddingRight = parseInt(j.css("padding-right").replace("px", "")), titlePaddingLeft = parseInt(f.css("padding-left").replace("px", "")), titlePaddingRight = parseInt(f.css("padding-right").replace("px", ""));
	if(l.css("display") != "none") {
...
Mobi.bindTopIconBack = function() {
	if(Fai.top._templateFrameId === 2004) {
		...
		a.off("click.tb").on("click.tb", function() {
			if(Fai.top._manageMode) {
				jm(Fai.top.document.getElementById("jz_designerOperate").querySelector(".j_designerBack")).click()
			} else {
				if(window.history.length <= 1) {
...
Mobi.videoPoster = function(c, b) {
	var a = jm("#" + c);
	a[0].addEventListener("loadedmetadata", function() {
		var d = a.height(),
			h = a.width(),
			e = d / _htmlFontSize,
			f = h / _htmlFontSize;
...
Mobi.initBgm = function(d) {
	...
	b.off("ended.bgm").on("ended.bgm", function(k) {
		var j = jm("#bgMusic .bgm_icon_inner");
	...
	if(jm.cookie("_bgmIsPause")) {
		...
		if(jm("#bgmAudio").attr("autoplay")) {
			...
			if(/(iPhone|iPad|iPod|Android)/i.test(c)) {
				...
				jm("body").off("touchstart.bgm").on("touchstart.bgm", function(j) {
					if(!parseInt(jm.cookie("_bgmIsPause")) && !(j.target == a[0])) {
						Mobi.bgmPlay()
					}
				});
				if(!jm("#bgmAudio").attr("loop")) {
					jm("#bgmAudio").off("playing.bgm").on("playing.bgm", function(j) {
	...
	jm("video").on("play", function() {
		Mobi.bgmPause();
		jm.cookie("_bgmIsPause", 1)
	});
	jm("video").on("pause", function() {
		if(h) {
			Mobi.bgmPlay();
			jm.cookie("_bgmIsPause", 0)
		}
...
Mobi.initBannerDefaultDom = function(s) {
	...
	for(k = 0; k < t; k++) {
		...
		l.push('<a href="' + (a != 0 ? jm.encodeHtmlAttr(q) : "javascript:;") + "\" vheight='" + e + "' vwidth='" + n + "' class='bannerImageDiv " + (o == 0 ? "systemtBanner" : "customBanner") + "' style='display:block; background-image:url(" + r + ");'></a>");
		if(f) {
...
Mobi.initSwipe = function(a, n, f) {
	var r = Mobi.getJQMobi();
	var u = document.querySelector("#" + a);
	var l = jm("#webBanner")[0];
	if(!u) {
		return
	}
	if(jm._swipehandle) {
	...
	var d = u.querySelectorAll(".bannerImageDiv.customBanner"),
		w = u.querySelectorAll(".bannerImageDiv"),
		x = d.length,
		h = w.length,
		k = u.offsetWidth,
		c = "",
		...
		p, y, z;
	for(var v = 0; v < x; v++) {
		p = parseInt(d[v].getAttribute("vwidth"));
		y = parseInt(d[v].getAttribute("vheight"));
		z = y / p;
		if(z > o) {
			o = z
		}
	...
	if(h === 0) {
		var q = Mobi.preciseDecimal(k * 300 / 640, 2);
		u.querySelector(".defaultBanner").style.height = q + "px"
	} else {
		c = s();
		u.style.height = c + "px";
		l.style.height = c + "px"
	...
	if(!f) {
		if(!Mobi.checkBrowser() || n.et == 1) {
			jm._swipehandle = r("#" + a).swipehandle(u, {
				auto: 3000,
				continuous: true,
				bulletsClick: true,
				speed: n.st * 100,
			...
			var j = u.getBoundingClientRect().width,
				b = u.children[0],
				t = b.children;
			var m = t.length;
			while(m--) {
				t[m].style.width = j + "px"
			}
		}
		if(!u.querySelector(".bullets")) {
			return
		}
		var e = u.querySelector(".bullets").getElementsByTagName("li");
		return
	}
	...
	function s() {
		...
		var B = (h - x > 0);
		if(B) {
			A = (k * o < k * 300 / 640 ? Mobi.preciseDecimal(k * 300 / 640, 2) : Mobi.preciseDecimal(k * o, 2))
		} else {
			A = Mobi.preciseDecimal(k * o, 2)
		}
		return A
	}
};
	...
	function I(ap) {
		...
		for(ak = 0; ak < al; ak++) {
			...
			if(aq == 0) {
				...
				ad.push('<a href="' + (af != 0 ? n.encodeHtmlAttr(ar) : "javascript:;") + "\" vheight='" + ah + "' vwidth='" + aa + "' class='bannerImageDiv " + (Z == 0 ? "systemtBanner" : "customBanner") + "' style='display:block; background-image:url(" + ag + ");'></a>")
			} else {
				if(aq == 4) {
					...
					for(ac = 1; ac < aq; ac++) {
						...
						ad.push("<a class='bannerImageDiv J_bannerItem systemtBanner billboardImgInner " + (Z == 0 ? "systemtBanner" : "customBanner") + "' hidefocus='true' title='' href=\"" + (af != 0 ? n.encodeHtmlAttr(ar) : "javascript:;") + "\" vheight='" + ah + "' vwidth='" + aa + "' target='' style='overflow: hidden; display: block; outline: none; margin: 0px auto; position: relative; z-index: 1; background-image: url(" + ag + "); background-position: 50% 50%; background-repeat: no-repeat;'>");
						ad.push("</a>");
						ad.push("</div>");
						ad.push("</div>")
					}
...
Mobi.initMallCoupon = function(c) {
	...
	b.find(".buttonWords").bind("click", function() {
		if(couponClickLock) {
			Mobi.clickCoupon(jm(this).parents(".coupon3"))
		}
	});
	b.find(".coupon1, .coupon2, .coupon3,.couponItem").bind("click", function() {
		if(couponClickLock) {
...
Mobi.initCouponPromotion = function(c) {
	...
	jm(b).find(".toMallIndex,.toMallIndexContent").bind("click", function() {
		Fai.top.location.href = "index.jsp"
	})
};
...
(function(b, e, a, d) {
	a.init = function(x, n, p) {
		...
		if(!k.mm) {
			b(s).one("webkitAnimationStart mozAnimationStart MSAnimationStart oanimationstart animationstart", function() {
		...
		if(!k.mm && k.i) {
			b(s).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function() {
...
(function(w, t, o) {
	...
	t.init = function(G, H, F) {
		...
		c = H;
		m = parseInt(F) || 0;
		a = "<div class='getMInfo'><div class='mListLoadingPic'><img src=" + Fai.top._resRoot + "/image/loading/loading_2.gif?v=201509211806></div><div class='mListLoadingTxt'>正在加载更多订单</div></div>";
		E(A);
		return true
	};
			...
			success: function(G) {
				...
				if(F > l) {
					...
					if(I <= 10 && I == z) {
						...
						e.find(".pagePrev").unbind().bind("click", function() {
							l--;
							B()
						})
					} else {
	...
	function D() {
		...
		if(G == 100 && G < r && l == 0) {
			...
			e.find(".pageSelect").val(l);
			e.find(".pageNext").unbind().bind("click", function() {
				l++;
				C()
			})
		} else {
	...
	function n() {
		e.find(".payButton").unbind().bind("click", function() {
...
Mobi.initShare = function(b, d, c) {
	var a = b;
	jm("#" + a).unbind("click").bind("click", function() {
		if(!Mobi.isWechat()) {
			var e = [];
...
Mobi.initFixAreaEvent = function() {
	var a = 0,
		b = jm(".J_mallNavBtn");
	b.on("click.clickAnimate", function() {
		var c = jm(this);
	...
	b.off("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend").on("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function(d) {
		var e = jm(this);
...
(function(b, a, d) {
	a.initFooter = function() {
		...
		function k(r) {
			...
			var p = r.find(".J_supportLink");
			var o = Fai.top._mjzHomeHost;
			var t = (Fai.top._isFreeVer === true);
			var q = t ? 2508 : 2509;
			var s = "//" + o + "/proTest.html?_ta=" + q;
			p.attr("href", s)
		}

		function j(q) {
			var p = q.find(".J_supportLink");
			var r = "touchend.logClick";
			var s = a.isPC();
			if(!s) {
				r = "click.logClick"
			}
			...
			function o(t) {
				if(e()) {
					a.logDog(l, f.CLICK_PAY)
				}
				if(h()) {
					a.logDog(l, f.CLICK_FREE)
				}
			}
		}

		function m(s) {
			var r = s.find(".J_supportLink");
			if(r.length === 0) {
				return
			}
			var v = b(window);
			var q = "scroll.footerSupportLinkExposure";
			var u = "resize.footerSupportLinkExposure";
			var t = a.throttle(p);
			v.off(q).on(q, t);
			v.off(u).on(u, t);
			p();
			...
			function p() {
				var w = a.isInViewArea(v, r);
				if(w) {
					if(e()) {
						a.logDog(l, f.EXPOSURE_PAY)
					}
					if(h()) {
						a.logDog(l, f.EXPOSURE_FREE)
					}
					o()
				}
			}
		...
		function h() {
			...
			return o && Fai.top._isFreeVer === true
		}
	};
...
Mobi.initTabModuleStyle = function(B, H) {
	var b = jm("#module" + B),
		...
		w = 0,
		C, o, j, q, v, m, z, F, f, y;
	t.off("mouseover.action").on("mouseover.action", function() {
		jm(this).addClass("formTabButtonHover")
	}).off("mouseleave.action").on("mouseleave.action", function() {
	...
	q = D.offset();
	D.find(".J_preBtn").on("click", function() {
		preBtn = jm(this);
	...
	D.find(".J_nextBtn").on("click", function() {
		z = jm(this);
...
Mobi.initSimpleTextContent = function(b, p, j) {
	var c = jm("#module" + b),
		k = c.attr("_inpopupzone") > 0,
		l = c.find(".fkEditor"),
		d = c.find(".simpleText"),
		n = parseInt(c.css("top") || 0),
		h = parseInt(c.css("left") || 0),
		f = j || _htmlFontSize,
		a = c.width().replace("rem", ""),
		o = 0,
		m = Fai.top._manageMode;
	element = "", cloneModule = "";
	AJump = l.find("a");
	if(Fai.top._manageMode) {
		var e = Fai.top.Mobi.getPopupZone();
		if(e && e.enterState == "preview") {
			m = false
		}
	}
	element = m ? l : d;
	if(p != 1 && p != 2) {
		if(c.is(":visible")) {
			...
			if(m) {
				o = parseInt(l.attr("_autowidth") || 1);
				o && (a = parseInt(l.attr("_width") || 180) / f)
			} else {
				parseInt(c.attr("_autowidth") || 1) && (a = 180 / f)
			}
		...
		!m && c.css("overflow", "visible") && c.find(".formMiddle").css("overflow", "visible")
	}
	if(m) {
		AJump.each(function(s, q) {
			var r = $(q).attr("href");
			if(r) {
		...
		c.find(".fk-editor a").each(function(r, s) {
			var q = jm(s).attr("_href");
			if(q) {
				jm(s).attr("href", q)
			}
		})
	}
	checkNode.traverseTagStyle(element, f)
};
var checkNode = {
...
Mobi.initFoldModuleStyle = function(a, f) {
	var b = jm("#module" + a),
		e = b.find(".J_formFoldButton"),
		l = b.find(".J_formFoldContentList"),
		j = jm(l).attr("_display"),
		m = parseInt(jm("html").css("fontSize")),
		d = 0;
	if(j == 0) {
		e.off("click.action").on("click.action", function() {
			var t = jm(this);
			var s = t.parent();
			var r = t.parent().find(".J_formFoldContent");
			var q = parseInt(r[0].offsetHeight) + parseInt(b.height());
			c(j, r);
			if(t.hasClass("active")) {
				h(t)
			} else {
				if(r.find(".form").hasClass("formStyle30")) {
					p(r)
				}
				o(s.parent());
				k(t)
		...
		if(j == 1) {
			e.off("click.action").on("click.action", function() {
				var s = jm(this);
				var r = s.parent();
				var q = s.parent().find(".J_formFoldContent");
				c(j, q);
				if(s.hasClass("active")) {
					h(s)
				} else {
					if(q.find(".form").hasClass("formStyle30")) {
						p(q)
					}
					k(s)
				}
	...
	if(e.length > 0 && e.parent().find(".form").length > 0) {
		n(d)
	}

	function k(u) {
		var r = u.parent();
		var t = r.find(".J_formFoldContent");
		var q = parseInt(t.find(".form")[0].offsetHeight);
		var s = q + b.height();
		u.addClass("active");
		t.css("height", q + "px");
		if(Fai.top._manageMode) {
			Mobi.removeMoveFrame("module" + a);
			Mobi.addMoveFrame("module" + a, undefined, s)
		}
		t.one("transitionend", v);
		t.one("webkitTransitionEnd", v);

		function v() {
			if(u.hasClass("active")) {
				t.off("transitionend", v);
				t.off("webkitTransitionEnd", v);
				t.css("height", "auto")
			}
		}
	}
	...
	function h(t) {
		...
		var r = t.parent();
		var s = r.find(".J_formFoldContent");
		var q = parseInt(s.find(".form")[0].offsetHeight);
		s.css("height", q + "px");
		setTimeout(function() {
	...
	function o(q) {
		q.find(".J_formFoldButton").each(function(s, r) {
			if(jm(r).hasClass("active")) {
				h(jm(r))
			}
		})
	}
	...
	function p(q) {
		q.find(".form").find(".mobiVideo").css("display", "block")
	}

	function c(q, r) {
		if(q == 1) {
			b.find(".form").find(".mobiVideo").each(function(s, t) {
				t.pause()
			});
			if(r.find(".form").hasClass("formStyle30")) {
				r.find(".form").find(".mobiVideo").css("display", "none")
			}
		} else {
...
(function(b, e, a, h) {
	...
	function f(k) {
		this.moduleId = k.moduleId;
		this.pgType = k.pgType;
		this.moduleStyle = k.moduleStyle;
		this.type = k.type;
		this.manageMode = k.manageMode;
		this.$module = b("#module" + this.moduleId);
	...
	f.prototype = {
		...
		init: function(l) {
			...
			if(this.type === "Product") {
				var k = parseInt(b.getCookie("pg_startX")) || 0;
				...
				if(this.type === "Photo") {
					var m = parseInt(b.getCookie("pg_startX_photo")) || 0;
		...
		initFirstGroupAllWidth: function() {
			var k = this;
			this.$firstGroupItemDom.each(function(l, m) {
				k.firstGroupAllWidth += (Math.ceil(b(m).width()) + k.firstGroupItemMarginWidth)
			});
			this.$firstGrouplistDom.css("width", k.firstGroupAllWidth + "px")
		},
		initSlide: function(l) {
			var k = new iScroll("J_first_group_" + this.moduleId, {
				x: l || 0,
				scrollX: true,
				scrollY: false,
				mouseWheel: true,
		...
		initFirstGroupHeight: function(m, l, k) {
			if(l > k) {
				this.$module.find("." + m + " .J_first_group").css({
					height: l + "px"
				})
			}
		},
		showLoading: function() {
			var l = this.moduleId;
			var k = "<div class='pageLoading J_pageLoading'><div class='fk-circle'><div class='fk-circle1 fk-child'></div><div class='fk-circle2 fk-child'></div><div class='fk-circle3 fk-child'></div><div class='fk-circle4 fk-child'></div><div class='fk-circle5 fk-child'></div><div class='fk-circle6 fk-child'></div><div class='fk-circle7 fk-child'></div><div class='fk-circle8 fk-child'></div><div class='fk-circle9 fk-child'></div></div></div>";
			this.$module.find(".productList" + l).empty().append(k).css("height", this.grouplistHeight + "px")
		},
		closeLoading: function() {
			this.$module.find(".J_pageLoading").remove()
		}
	};
	...
	function d(m) {
		...
		var x = m.moduleStyle;
		var p = m.pgType;
		var v = m.$module.find("#productList" + l)[0].clientHeight;
		var y = m.$module.find("#productList" + l);
		var q = m.$firstGrouplistDom;
		var u = false;
		var o;
		var z;
		var w;
		...
		try {
			n = e.top._colInfo.id;
			s = e.top._colInfo.extId
		} catch(t) {
			n = 3;
			s = 0
		}
		m.initFirstGroupHeight("J_verticalGroupHasProduct", v, m.grouplistHeight);
		q.off("click.J_first_groupitem").on("click.J_first_groupitem", ".J_first_groupitem", function(A) {
			if(!b(this).hasClass("selected")) {
				if(x === 4) {
					r = parseInt(q.css("transform").match(/translate3d\((.*)\)/)[1].split(",")[0]);
					b.setCookie("pg_startX", r)
				}
				q.find(".J_first_groupitem").removeClass("selected");
				b(this).addClass("selected");
				if(p === 0 || p === 1) {
					o = parseInt(b(this).attr("data-id"));
					w = y.find('div[gid="' + o + '"]');
					k = "index.jsp?groupId=" + o
				} else {
					if(p === 2) {
						z = b(this).attr("data-cn");
						w = y.find("div[gcn=" + z + "]");
						k = "index.jsp?groupName=" + z
					}
				}
				y.find(".J_groupItem").hide();
				setTimeout(function() {
					if(!u) {
						m.showLoading()
					}
				...
				b.ajax({
					type: "post",
					url: "ajax/ajaxLoadModuleDom_h.jsp",
					data: "cmd=getAjaxPageModuleInfo&_colId=" + n + "&_extId=" + s + "&moduleId=" + l + "&href=" + b.encodeUrl(k),
					error: function(B) {
						Mobi.ing(LS.systemError)
					},
					success: function(C, G, D) {
						var B = D.getResponseHeader("wafUpdate");
						if(B && B == "true") {
							window.location.reload()
						}
						u = true;
						...
						var C = b.parseJSON(C);
						if(C.success) {
							var F = b(C.domStr).find(".middleCenter" + l).html();
							var E = b("#module" + l).find(".middleCenter" + l);
							E.html(F);
							b.fkEval.call(window, C.scripts)
						} else {
							Mobi.ing(C.msg)
						}
					}
	...
	function c(l) {
		...
		var s = l.moduleStyle;
		var n = l.pgType;
		var o = l.$firstGrouplistDom;
		var k = l.moduleId;
		var q = r.find("#groupList" + k)[0].clientHeight;
		var m = r.find("#groupList" + k);
		var p = m.attr("_prUrl") || "";
		l.initFirstGroupHeight("J_verticalGroupHasPhoto", q, l.grouplistHeight);
		o.off("click.J_first_groupitem").on("click.J_first_groupitem", ".J_first_groupitem", function(x) {
			var v;
			var y;
			var w;
			var u;
			var t = 0;
			if(s === 4) {
				t = parseInt(o.css("transform").match(/translate3d\((.*)\)/)[1].split(",")[0]);
				b.setCookie("pg_startX_photo", t)
			}
			if(n === 0) {
				v = parseInt(b(this).attr("data-id"));
				u = r.find("#sed_" + v)
			} else {
				if(n === 2) {
					y = b(this).attr("data-cn");
					u = r.find(".J_secondGroupPanel[gcn='" + y + "']")
				}
			}
			w = r.find(".J_secondGroupPanel");
			if(!u.html().trim() && p) {
				window.location.href = p + "?groupId=" + v;
				return false
			}
			o.find(".J_first_groupitem").removeClass("selected");
			b(this).addClass("selected");
			w.removeClass("selected");
			u.addClass("selected");
			q = r.find("#groupList" + k)[0].clientHeight;
			l.initFirstGroupHeight("J_verticalGroupHasPhoto", q, l.grouplistHeight)
		});
		j(m, this)
	}
	...
	function j(k, n) {
		if(!n.manageMode || n.moduleStyle === 7) {
			return
		}
		var l = k.find(".J_groupItem");
		var o = {
			key: "productGroup",
			targets: l,
			type: Mobi.QuickIn.TYPE.FU,
		...
		function m(w, q) {
			if(w.length == 0 || !q.fileId || q.fileId.length == 0) {
				return
			}
			var u = parseInt(b(w).parent().attr("data-id"));
			var p = q.fileId;
			var t = q.filePath;
			var v = q.isDefaultColor;
			var r = q.color;
			var s = "ajax/productGroup_h.jsp?cmd=setSingle&id=" + u + "&mIcon=" + p;
			if(!v) {
				s = s + "&mIconColor=" + e.top.Fai.encodeUrl(r)
			}
			b.ajax({
				url: s,
				type: "get",
				...
				success: function(x) {
					var A = $.parseJSON(x);
					if(A.success) {
						var B = b(w).parent().find(".J_itemImage");
						var z = b(B).parent();
						var y = "f-itemImg";
						z.empty();
						if(top.Fai.isFontIcon(p)) {
							if(!v) {
								z.append("<span class='J_itemImage isFontIcon " + y + " " + t + "' style='color:" + r + "'></span>")
							} else {
								z.append("<span class='J_itemImage isFontIcon " + y + " " + t + "'></span>")
							}
						} else {
							z.append("<div class='J_itemImage " + y + "' style='background-image: url(" + t + ");' data-w='" + q.fileWidth + "' data-h='" + q.fileHeight + "' />")
						}
						top.Fai.ing(A.msg, true)
					} else {
						top.Fai.ing(A.msg, true)
					}
				}
...
})(jm, Fai, Mobi.initModuleProductGroup || (Mobi.initModuleProductGroup = {}));
(function(d, a, f) {
	var b = false;
	a.PopupZone = function(j, m, k, h, l) {
		var m = m || (j && j.attr("id").replace("module", "")),
			j = j || (m && d("#module" + m));
		this.id = m;
		this.module = j;
		this.moduleInfo = l || {};
		this.create = h || false;
		this.funPanel = null;
		this.enterState = k || "readOnly", this.allPopupZoneList = [];
		this.historyModules = [];
		this.inModuleListInfo = [];
		this.delModuleIdList = [];
		this.switchActive = false;
		this.hasSave = false
	};
	a.closeZone = function() {
		var h = a.pZone;
		if(!h) {
			return
		}
		if(Fai.top._manageMode) {
			if(h && h.enterState == "edit") {
				return
			}
			h.stateClassDeal()
		}
	...
	a.open = function(l, j, h) {
		a.pZone && a.pZone.destroy();
		h = h || (Fai.top._manageMode ? "preview" : "readOnly");
		a.pZone = new a.PopupZone(null, l, h);
		var k = a.pZone;
		k.initDom(j, k.show, true);
		a.id = a.pZone.id
	};
	a.PopupZone.prototype.show = function() {
		if(this.enterState == "readOnly") {
			this.openPopupZone()
		} else {
			if(this.enterState == "edit") {
				this.goEdit(this.module)
			} else {
				if(this.enterState == "preview") {
					this.goPreview(this.module)
				}
			}
		}
		this.bindClikckMaskToClose()
	};
	a.PopupZone.prototype.openPopupZone = function() {
		a.pZone && a.pZone.destroy();
		var h = "";
		h += "<div class='jz_popupZone_wrap'>";
		h += "<div class='jz_popupZone_content'>";
		h += this.module[0].outerHTML;
		h += "</div>";
		h += "</div>";
		d("#g_web").append(h);
		d("#g_body").addClass("jz_popupZoneShow");
		d("html").addClass("jz_popupZoneShow2");
		d.fkEval.call(window, this.script);
		e();
		this.initCloseBtn();
		this.bindBeforeUnload()
	};
	a.PopupZone.prototype.initDom = function(l, k, j, h) {
		if(l) {
			this.module = d(l);
			k.apply(this)
		} else {
	...
	a.PopupZone.prototype.ajaxDom = function(k, j, h) {
		if(b) {
			return
		}
		b = true;
		var l = [];
		l.push("cmd=", "getPopupZoneModule");
		l.push("&_fresh=", "false");
		l.push("&_colId=", Fai.top._pageId);
		l.push("&_extId=", Fai.top._extId);
		l.push("&popupZoneId=", this.id);
		l.push("&manageMode=", Fai.top._manageMode);
		l.push("&onlyLoadVisitScript=", !!j);
		if(!!h) {
			!!h.lType && l.push("lType=", h.lType);
			!!h.lName && l.push("lName=", h.lName)
		}
		d.ajax({
			type: "post",
			url: "ajax/mobimodule_h.jsp",
			data: l.join(""),
			error: function() {
				Fai.top.Fai.ing("系统异常，请稍后重试", true)
			},
			success: function(m) {
				var o = d.parseJSON(m);
				var p = d.parseJSON(o.rtInfo);
				var r = a.pZone;
				var q;
				if(!p) {
					return
				...
				r.module = d(p.dom);
				r.moduleInfo = p.moduleInfo;
				r.script = p.script;
				if(Fai.top._manageMode) {
					r.inModuleListInfo = p.inZoneModuleInfoList;
					for(var n in p.inModuleAttr) {
						Fai.top.moduleAttr[n] = p.inModuleAttr[n]
					}
					r.clickToClose = !Fai.top.Fai.checkBit(p.moduleInfo.flag, 1)
				}
				k && k.apply(r);
				b = false
			}
	...
	a.PopupZone.prototype.destroy = function() {
		d(".jz_popupZone_wrap").remove();
		d("#g_body").removeClass("jz_popupZoneShow");
		d("html").removeClass("jz_popupZoneShow2");
		if(Fai.top._manageMode) {
			this.manageDestroy()
		}
	};
	a.PopupZone.prototype.bindClikckMaskToClose = function() {
		var j = !c(this.moduleInfo.flag, 1);
		var h = d(".jz_popupZone_wrap");
		h.off("click.close");
		if(!j) {
			return
		}
		h.on("click.close", function(l) {
			var k = d(l.target);
			if(!k.hasClass("J_formPopupZoneContent") && k.closest(".J_formPopupZoneContent").length > 0) {
				return
			}
			a.closeZone()
		})
	};
	a.PopupZone.prototype.initCloseBtn = function() {
		var h = this.moduleInfo.content.cmi;
		d("#module" + this.id).find("#module" + h).removeClass("popupZoneCloseBtn").addClass("popupZoneCloseBtn")
	};
	a.PopupZone.prototype.bindBeforeUnload = function() {
		if(Fai.top._manageMode) {
			return
		}
		var h = d("#module" + this.id);
		h.find("a").off("click.zone").on("click.zone", function() {
			var j = d(this).attr("href");
			if(!j || j == "" || j == "javascript:void(0)" || j == "javascript: void(0)" || j == "javascript:;") {
				return
			}
			a.closeZone()
		})
	...
	function c(r, j) {
		var n = true;
		if(r > 2147483647 || r < 0 || j > 2147483647 || j < 0) {
			n = false
		}
		if(n) {
			return(r & j) == j
		...
		var p = r.toString(2);
		var l = j.toString(2);
		if(p.length > 62 || l.length > 62) {
			alert("Does not support more than 62 bit. flagBinary.length=" + p.length + ",bitFlagBinary.length" + l.length + ".");
			return false
		}
		var s;
		var q;
		...
		o = s = q = m = 0;
		if(p.length > 31) {
			k = p.slice(0, p.length - 31);
			h = p.slice(p.length - 31);
			o = parseInt(k, "2");
			s = parseInt(h, "2")
		} else {
			s = parseInt(p.slice(0, p.length), "2")
		}
		if(l.length > 31) {
			k = l.slice(0, l.length - 31);
			h = l.slice(l.length - 31);
			q = parseInt(k, "2");
			m = parseInt(h, "2")
		} else {
			m = parseInt(l.slice(0, l.length), "2")
		}
		var t = (s & m) == m;
		if(t) {
			t = (o & q) == q
	...
	function e() {
		var k = a.pZone,
			h = window.scrollY,
			j = 0;
		d("#module" + k.id).find(".form").each(function(m, n) {
			var l = d(n).offset().bottom - h;
			j = l > j ? l : j
		});
		if(j > 0) {
			d(".J_formPopupZoneContent").height(j + "px")
		}
	}
})(jm, Mobi._popupZone || (Mobi._popupZone = {}));
(function(c, a, d) {
	var b;
	a.initBaiDuMap = function(j, o, n) {
		try {
			var e = new BMap.Map(j);
			var p = new BMap.Point(o.x, o.y);
			e.centerAndZoom(p, o.z);
			e.enableDoubleClickZoom();
			e.enablePinchToZoom();
			e.enableDoubleClickZoom();
			var q = new BMap.Icon(Fai.top._resRoot + "/image/onlineMap/marker_red_sprite.png?v=201809101152", new BMap.Size(22, 25), {
				anchor: new BMap.Size(10, 25),
			});
			var k = new BMap.Marker(p, {
				icon: q
			});
			e.addOverlay(k);
			var m = new BMap.NavigationControl();
			e.addControl(m);
			if(!b) {
				b = a.addComplexCustomOverlay(BMap)
			}
			var f = c("#" + j);
			var l = new b({
				point: p,
				content: '<div class="msg">' + n + "</div>",
				boxClass: "msgBox",
				arrowClass: "arrow",
				zIndex: 1,
				offset: {
			...
			e.addOverlay(l);
			a.initMapEvent(e, f, k, p, l)
		} catch(h) {
			a.loadMapFailure(j, h.name)
		}
	};
	a.initMapEvent = function(j, h, f, e, k) {
		if(Fai.top._manageMode) {
			h.on("mousedown", function(l) {
				l.stopPropagation()
			})
		}
		j.addEventListener("click", function(l) {
			if(l.overlay) {
				l.overlay.addEventListener("infowindowopen", function(m) {
					j.removeOverlay(k)
				})
			}
		});
	...
	a.loadMapFailure = function(h, f) {
		var e = "<div class='mapLoadError'><div class='errormsg'>地图加载失败</div><div class='errName'>" + f + "</div></div>";
		c("#" + h).html(e)
	};
	a.addComplexCustomOverlay = function(j) {
		function h(l) {
			this.point = l.point;
			this.content = l.content || "";
			this.zIndex = l.zIndex || 1;
			this.boxClass = l.boxClass || "";
			this.boxStyle = k(l.boxStyle);
			this.arrowClass = l.arrowClass || "";
			this.arrowStyle = k(l.arrowStyle);
			this.offset = l.offset || {
				x: 0,
				y: 0
			}
		}
		h.prototype = new j.Overlay();
		h.prototype.initialize = function(l) {
			this.map = l;
			...
			l.getPanes().labelPane.appendChild(this.box);
			this.initDefaultEvent(l);
			return this.box
		};
		h.prototype.addEventListener = function(m, l) {
			this.box.addEventListener(m, l)
		};
		h.prototype.initDefaultEvent = function(m) {
			var l = this;
			this.addEventListener("mousedown", function(n) {
				n.stopPropagation()
			});
			this.addEventListener("click", function(n) {
				n.stopPropagation()
			});
			this.addEventListener("touchmove", function(n) {
				n.stopPropagation()
			});
			this.addEventListener("touchstart", function(n) {
				if(n.target.getAttribute("class").indexOf("close") != -1) {
					m.removeOverlay(l)
				}
			});
			this.addEventListener("mousedown", function(n) {
		...
		function f(m) {
			var l = m.map.pointToOverlayPixel(m.point);
			m.box.style.left = l.x - parseInt(e(m.box, "width")) / 2 - (parseInt(e(m.box, "padding-left")) + parseInt(e(m.box, "padding-right"))) / 2 + m.offset.x + "px";
			m.box.style.bottom = -l.y + parseInt(e(m.arrow, "height")) - m.offset.y + "px";
			m.arrow.style.left = parseInt(e(m.box, "width")) / 2 + 9 + "px"
		}

		function e(m, l) {
			if(typeof getComputedStyle != "undefined") {
				var n = getComputedStyle(m, null).getPropertyValue(l);
				return l == "opacity" ? n * 100 : n
			} else {
				if(typeof m.currentStyle != "undefined") {
					if(l == "opacity") {
						return Number(m.currentStyle.getAttribute("filter").match(/(?:opacity[=:])(\d+)/)[1])
					} else {
						return m.currentStyle.getAttribute(l)
					}
				}
			}
		}
...
})(jm, Mobi.mobiBaiduMap || (Mobi.mobiBaiduMap = {}));
...
Mobi.scrollNavFix = function() {
	...
	document.addEventListener("touchstart", function(j) {
		a()
	}, false);
	...
	document.addEventListener("touchcancel", function(j) {
		a()
	}, false);
	document.addEventListener("scroll", function() {
		a()
...
Mobi.initMobiPage = function(a) {
	...
	setTimeout(function() {
		jm("#jiaxin-mcs-fixed-btn").off("click.log").on("click.log", function() {
...
Mobi.isFromJzmHomeIframe = function() {
	return Mobi.getUrlParam(window.location.href, "_fromJzmHome") == "true"
};
Mobi.fixJzmHomeIframeNest = function() {
	if(Mobi.isFromJzmHomeIframe()) {
		jm("#mobiCornerAd").addClass("jz_foreHideCornerAd");
		jm("#g_body").delegate("a", "click", function(b) {
			var a = jm(this);
			Mobi.fixJzmHomeIframeParam(a, a.attr("href"))
		})
	}
};
Mobi.fixJzmHomeIframeParam = function(d, c) {
	if(!Mobi.isFromJzmHomeIframe()) {
		return c
	}
	if(typeof c != "string") {
	...
	if(c.indexOf("mailto") >= 0 || c.indexOf("sms") >= 0 || c == "#" || c.indexOf("javascript:") >= 0 || c.indexOf("tel") >= 0) {
		return c
	}
	var b = c.indexOf("?") < 0 ? "?" : "&",
		a = c + b + "_fromJzmHome=true";
	d.attr("href", a);
	return a
};
...
Mobi.setCtrlStyle = function(b, a) {
	...
	e.push(a);
	jm("head").append('<style type="text/css" id="' + b + '">' + e.join("\n") + "</style>")
};
...
Mobi.ajaxLoadModuleDom = function(e, b, f) {
	var a = window.location.href,
		d = "",
		c = "";
	if(a.indexOf("lcid") > 0) {
		var j = Mobi.getUrlParam(a, "lcid");
		if(j) {
			c += "?lcid=" + j
		}
	}
	if(a.indexOf("_fromJzmHome") > 0) {
		var h = Mobi.getUrlParam(a, "_fromJzmHome");
		if(h) {
			d += "&_fromJzmHome=true"
		}
	}
	jm.ajax({
		...
		data: "cmd=loadModuleDom&colId=" + e + "&extId=" + b + "&ajaxOptionInfo=" + jm.encodeUrl(jm.toJSON(f)) + d,
		error: function() {
			
		},
		success: function(u, m, p) {
			var s = p.getResponseHeader("wafUpdate");
			if(s && s == "true") {
				window.location.reload()
			}
			var u = jm.parseJSON(u);
			if(u.success) {
				var q = jm.parseJSON(u.rtInfo);
				if(q.topBar) {
					var t = Mobi.getPreviewWindow().jm(".multilingual");
					t.children().remove();
					t.append(q.topBar.dom);
					jm.fkEval.call(window, q.topBar.scripts);
					Mobi.initLocater()
				}
				if(q.moduleDomList && q.moduleDomList.length > 0) {
					for(var r = 0; r < q.moduleDomList.length; r++) {
						var l = q.moduleDomList[r];
						var k = l.moduleId;
						var o = l.dom;
						var n = l.scripts;
						loadWholeModuleDom(k, o, n)
					}
					Mobi.initFooterHeight();
					Mobi.onlineServiceInputWord()
				}
...
Mobi.checkNestModule = function(c) {
	...
	inPopupZone = parseInt(c.attr("_inpopupzone")) || 0;
...
Mobi.limitUv = function() {
	e();
	if(Fai.top.usedUv >= 500) {
		Mobi.logDog(200618, 14);
		var d = '<div class="tipDialog"><span class="quitIcon"></span><img class="tipComtent" src="' + Fai.top._resImageRoot + '/image/v2/pageNotFound.png"></img><div class="tipFont"> 网站流量已达上限，暂时无法访问</div><div class="tipButton"> 升级网站，拥有无限流量</div></div>';
		jm("#g_body .fk-limtUv").append(d);
		var b = document.getElementById("g_body");
		b.style.overflowY = "hidden";
		jm(".fk-limtUv .quitIcon").click(function() {
			top.window.close()
		});
		jm(".fk-limtUv .tipButton").click(function() {
			Mobi.logDog(200618, 15);
	...
	function c() {
		...
		return(new Date([h, j, "01"].join("-")).getTime()) / 1000
	}
	...
	function a() {
		...
		var l = h.getMonth();
		var k = ++l;
		var f = new Date(h.getFullYear(), k, 1);
		var j = 1000 * 60 * 60 * 24;
		return(new Date(f - j).getTime()) / 1000
	}

	function e() {
		var h = "&typeQy=0&isMobile=1&siteLan=" + Fai.top._wid + "&startTime=" + c() + "&endTime=" + a();
		var f = 0;
		jm.ajax({
			type: "post",
			url: "/ajax/mobistatistics_h.jsp?cmd=timeDataNoAuth",
			data: h,
			timeout: 60000,
			async: false,
			error: function(l, j, k) {
				if(j == "timeout") {
					Fai.ing("抱歉，数据量过大查询失败。请稍后再试")
				}
			},
			success: function(k) {
				var j = jm.parseJSON(k);
				if(j.success) {
					Fai.top.usedUv = j.iVisitors
				}
			}
		})
	}
};
Mobi.limitDayUv = function() {
	var b = "&siteLan=" + Fai.top._wid + "&siteId=" + Fai.top._siteId;
	var a = 0;
	jm.ajax({
		type: "post",
		url: "/ajax/site_h.jsp?cmd=getLimitPreview",
		data: b,
		timeout: 60000,
		success: function(j) {
			var d = jm.parseJSON(j);
			if(d.success && d.type != 0) {
				var c = "";
				if(d.type == 1) {
					c += "免费域名" + d.acct + "只用作预览访问，商业使用请绑定独立域名";
					var h = '<div id="fk-tipsLimitUv" class="fk-tipsDiv"><div  class="fk-mobiTips"><div class="scroll"><li>' + c + "</li><li>" + c + "</li></div></div></div>";
					jm("body").prepend(h);
					Fai.top.scollDistance = 0;

					function k() {
						if(Fai.top.scollDistance == -700) {
							Fai.top.scollDistance = 0
						}
					...
					setInterval(k, 20);
					Mobi.autoFixGWebTop(_templateLayoutId)
				} else {
					c += "访客量超出限制，免费域名" + d.acct + "只用作预览访问，商业使用请绑定独立域名访问。";
					var e = "";
					if(d.ver == 0) {
						e += "升级网站,绑定域名"
					} else {
					...
					var h = '<div class="fk-limtUv"><div class="tipDialog"><span class="quitIcon"></span><img width="440px" height="245px" class="tipComtent" src="' + Fai.top._resImageRoot + '/image/v2/pageNotFound.png"></img><div class="tipFont" style="font-size:16px;width:90%;margin-left:5%;"> ' + c + '</div><div class="tipButton tipButton2">' + e + "</div></div></div>";
					jm("body").append(h);
					jm(".fk-limtUv .tipButton").click(function() {
						if(d.ver == 0) {
							window.open("/")
							...
							if(d.isFaiDomain) {
								window.open(Fai.top.httpStr + d._mainUrl)
							} else {
								window.open("https://i.fkw.com/portal.jsp")
							}
						}
					});
					var f = document.getElementById("g_body");
...
Mobi.ajaxLogDog = function() {
	jm.ajax({
		type: "POST",
		url: "/ajax/log_h.jsp?cmd=mobiLogDog&hostName=" + Fai.top.location.host,
		error: function() {
